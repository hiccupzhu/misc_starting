...
void SetIsUSBDevice(PDEVICE_OBJECT fdo,PDEVICE_EXTENSION pdx);
...
void SetIsUSBDevice(PDEVICE_OBJECT fdo,PDEVICE_EXTENSION pdx)
{
	...
	driverName1.Length=0;
	driverName1.Buffer=(PWSTR)ExAllocatePool(PagedPool,BUFFERSIZE);
	driverName1.MaximumLength=BUFFERSIZE;
	
	UNICODE_STRING driverName2;
	RtlCopyUnicodeString(&driverName1,&fdo->AttachedDevice->DriverObject->DriverName);
	KdPrint(("%ws\n",driverName1.Buffer));
	RtlInitUnicodeString(&driverName2,L"\\Driver\\USBSTOR");
	if(!RtlCompareUnicodeString(&driverName2,&driverName1,FALSE))
	{
		pdx->isUSBDevice=TRUE;
		KdPrint(("\\\\\t USBDevice=true\n"));
	}
	else
	{
		KdPrint(("\\\\\t USBDevice=false\n"));
	}
	
	ExFreePool(driverName1.Buffer);
	driverName1.Buffer=NULL;
	driverName1.Length=0;
	driverName1.MaximumLength=0;
                   ...
                   IN PVOID Context )
{
	...
	if(pdx->isUSBDevice && opCode==SCSIOP_MODE_SENSE  && CurSrb->DataBuffer 
