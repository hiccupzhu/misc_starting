...

{
	...
	if (abs(pLeft[0] - pDiag[0]) < abs(pDiag[0] - pTop[0])) {
		*acpred_direction = 1;	/* vertical */
		...
		for (i = 1; i < 8; i++) {
			predictors[i] = pTop[i];//rescale(top_quant, current_quant, pTop[i]);
		}
	} else {
		*acpred_direction = 2;	/* horizontal */
		predictors[0] = DIV_DIV(pLeft[0], iDcScaler);
		for (i = 1; i < 8; i++) {
			predictors[i] = pLeft[i + 7];//rescale(left_quant, current_quant, pLeft[i + 7]);
		}
	}
}
...
uint32_t calc_cbp_1(const int16_t codes[6 * 64])
{
	int i, j;
	uint32_t cbp = 0;
...
#else
			predict_acdc_asm_dm642(frame->mbs, x, y, mb_width, j, &qcoeff[j * 64],
						 iQuant, iDcScaler, predictors[j], 0);
#endif
...
#else
			S += calc_acdc_coeff_asm_dm642(pMB, j, &qcoeff[j * 64], iDcScaler, predictors[j]);
#endif
		}
...
#if 1
		pMB->cbp = calc_cbp_1(qcoeff);
		//printf("y=%d,x=%d,cbp=%d",y,x,pMB->cbp);
#else
		//pMB->cbp = calc_cbp_asm(qcoeff);
		pMB->cbp = calc_cbp_asm_642(qcoeff);
		//calc_cbp_asm_642
		//printf("y=%d,x=%d,cbp=%d\n",y,x,pMB->cbp);
#endif
	}
}
