   ...
   //Encode
		...
		if (bSaveAVI){
			...
			AVIStreamWrite(pMainFrame->ps,pMainFrame->m_Frame, 1, (LPBYTE)buf1,
						 FrameSize,/*lpbiTmp->bmiHeader.biSizeImage*/AVIIF_KEYFRAME,NULL,NULL);
		}
		////////////////////////////////
   //Decode
		ICDecompress(hic2,0,&lpbiTmp->bmiHeader,buf1,&lpbiOut->bmiHeader,&bufo[40]);
		//dec_main(buf, &bufo[40], nStreamLength, &nOstreamSize,352);
	} else {
		memcpy(&bufo[40],bufi,lpVHdr->dwBytesUsed);
