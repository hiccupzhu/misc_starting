 ...
 *
 * $Id: H264.c,v 1.65.2.5 2007/06/27 18:57:42 Isibaar Exp $
 *
 ****************************************************************************/
...
#if defined(_DEBUG)
unsigned int H264_debug = 0; /* H264 debug mask */
#endif
...
detect_cpu_flags(void)
{
	...
	unsigned int cpu_flags = H264_CPU_ASM;

#if defined(ARCH_IS_IA32) || defined(ARCH_IS_X86_64)
	cpu_flags |= check_cpu_features();
	if ((cpu_flags & H264_CPU_SSE) && sigill_check(sse_os_trigger))
		cpu_flags &= ~H264_CPU_SSE;

	if ((cpu_flags & H264_CPU_SSE2) && sigill_check(sse2_os_trigger))
		cpu_flags &= ~H264_CPU_SSE2;
#endif

#if defined(ARCH_IS_PPC)
	if (!sigill_check(altivec_trigger))
		cpu_flags |= H264_CPU_ALTIVEC;
#endif
...
/*****************************************************************************
 * H264 Init Entry point
 *
 * Well this function initialize all internal function pointers according
 * to the CPU features forced by the library client or autodetected (depending
 * on the H264_CPU_FORCE flag). It also initializes vlc coding tables and all
 * image colorspace transformation tables.
 *
 * Returned value : H264_ERR_OK
 *                  + API_VERSION in the input H264_INIT_PARAM structure
 *                  + core build  "   "    "       "               "
 *
 ****************************************************************************/
...
static
int H264_gbl_init(H264_gbl_init_t * init)
{
	unsigned int cpu_flags;

	if (H264_VERSION_MAJOR(init->version) != 1) /* v1.x.x */
		return H264_ERR_VERSION;

	cpu_flags = (init->cpu_flags & H264_CPU_FORCE) ? init->cpu_flags : detect_cpu_flags();
	...
	H264_QP_Add_Funcs = &H264_QP_Add_Funcs_C;
	H264_Init_QP();

	/* Quantization functions */
	quant_h264_intra   = quant_h264_intra_c;
	quant_h264_inter   = quant_h264_inter_c;
	dequant_h264_intra = dequant_h264_intra_c;
	dequant_h264_inter = dequant_h264_inter_c;
	...
	if ((cpu_flags & H264_CPU_MMX) || (cpu_flags & H264_CPU_MMXEXT) ||
		(cpu_flags & H264_CPU_3DNOW) || (cpu_flags & H264_CPU_3DNOWEXT) ||
		(cpu_flags & H264_CPU_SSE) || (cpu_flags & H264_CPU_SSE2))
	{
		/* Restore FPU context : emms_c is a nop functions */
		emms = emms_mmx;
	}

	if ((cpu_flags & H264_CPU_MMX)) {
		...
		H264_QP_Add_Funcs = &H264_QP_Add_Funcs_mmx;

		/* Quantization related functions */
		quant_h264_intra   = quant_h264_intra_mmx;
		quant_h264_inter   = quant_h264_inter_mmx;
		dequant_h264_intra = dequant_h264_intra_mmx;
		dequant_h264_inter = dequant_h264_inter_mmx;
	...
	if ((cpu_flags & H264_CPU_MMXEXT)) {
		...
		dequant_h264_intra = dequant_h264_intra_xmm;
		dequant_h264_inter = dequant_h264_inter_xmm;
	...
	if ((cpu_flags & H264_CPU_3DNOWEXT)) {
		...
		if ((cpu_flags & H264_CPU_MMXEXT)) {
			...
			/* Quantization */
			quant_h264_intra = quant_h264_intra_3dne;		/* cmov only */
			quant_h264_inter = quant_h264_inter_3dne;
			dequant_mpeg_intra = dequant_mpeg_intra_3dne;	/* cmov only */
			dequant_mpeg_inter = dequant_mpeg_inter_3dne;
			dequant_h264_intra = dequant_h264_intra_3dne;
			dequant_h264_inter = dequant_h264_inter_3dne;
	...
	if ((cpu_flags & H264_CPU_SSE2)) {

		calc_cbp = calc_cbp_sse2;

		/* Quantization */
		quant_h264_intra   = quant_h264_intra_sse2;
		quant_h264_inter   = quant_h264_inter_sse2;
		dequant_h264_intra = dequant_h264_intra_sse2;
		dequant_h264_inter = dequant_h264_inter_sse2;
	...
	if ((cpu_flags & H264_CPU_ASM)) { /* use assembler routines? */
	  idct_ia64_init();
	  fdct = fdct_ia64;
...
/*	  Halfpel8_Refine = Halfpel8_Refine_ia64; */
	  quant_h264_intra = quant_h264_intra_ia64;
	  quant_h264_inter = quant_h264_inter_ia64;
	  dequant_h264_intra = dequant_h264_intra_ia64;
	  dequant_h264_inter = dequant_h264_inter_ia64;
	...
	if ((cpu_flags & H264_CPU_ALTIVEC)) {
          ...
          /* Quantization */
          quant_h264_intra = quant_h264_intra_altivec_c;
          quant_h264_inter = quant_h264_inter_altivec_c;
          dequant_h264_intra = dequant_h264_intra_altivec_c;
          dequant_h264_inter = dequant_h264_inter_altivec_c;
		  ...
		  H264_QP_Add_Funcs = &H264_QP_Add_Funcs_Altivec_C;
        }
#endif

#if defined(ARCH_IS_X86_64)
	/* For now, only H264_CPU_ASM is looked for, so user can still
	...
	if (cpu_flags & H264_CPU_ASM) {
		...
		/* Quantization related functions */
		quant_h264_intra   = quant_h264_intra_x86_64;
		quant_h264_inter   = quant_h264_inter_x86_64;
		dequant_h264_intra = dequant_h264_intra_x86_64;
		dequant_h264_inter = dequant_h264_inter_x86_64;
		...
		H264_QP_Add_Funcs = &H264_QP_Add_Funcs_x86_64;
...
static int
H264_gbl_info(H264_gbl_info_t * info)
{
	if (H264_VERSION_MAJOR(info->version) != 1) /* v1.x.x */
		return H264_ERR_VERSION;

	info->actual_version = H264_VERSION;
	info->build = "H264-1.1.3";
...
static int
H264_gbl_convert(H264_gbl_convert_t* convert)
{
	int width;
	int height;
	int width2;
	...
	if (H264_VERSION_MAJOR(convert->version) != 1)   /* v1.x.x */
	      return H264_ERR_VERSION;

#if 0
	const int flip1 = (convert->input.colorspace & H264_CSP_VFLIP) ^ (convert->output.colorspace & H264_CSP_VFLIP);
#endif
	width = convert->width;
	...
	switch (convert->input.csp & ~H264_CSP_VFLIP)
	{
		case H264_CSP_YV12 :
		  ...
		  void *param2)
{
	...
	{
		case H264_GBL_INIT :
			return H264_gbl_init((H264_gbl_init_t*)param1);

        case H264_GBL_INFO :
            return H264_gbl_info((H264_gbl_info_t*)param1);

		case H264_GBL_CONVERT :
			return H264_gbl_convert((H264_gbl_convert_t*)param1);
...
/*****************************************************************************
 * H264 Native decoder entry point
 *
 * This function is just a wrapper to all the option cases.
 *
 * Returned values : H264_ERR_FAIL when opt is invalid
 *                   else returns the wrapped function result
 *
 ****************************************************************************/

int
H264_decore(void *handle,
			int opt,
			void *param1,
			void *param2)
{
	...
	case H264_DEC_CREATE:
		return decoder_create((H264_dec_create_t *) param1);

	case H264_DEC_DESTROY:
		return decoder_destroy((DECODER *) handle);

	case H264_DEC_DECODE:
		return decoder_decode((DECODER *) handle, (H264_dec_frame_t *) param1, (H264_dec_stats_t*) param2);
...
/*****************************************************************************
 * H264 Native encoder entry point
 *
 * This function is just a wrapper to all the option cases.
 *
 * Returned values : H264_ERR_FAIL when opt is invalid
...
int
H264_encore(void *handle,
			int opt,
			void *param1,
			void *param2)
{
	switch (opt) {
	case H264_ENC_ENCODE:

		return enc_encode((Encoder *) handle,
							  (H264_enc_frame_t *) param1,
							  (H264_enc_stats_t *) param2);

	case H264_ENC_CREATE:
		return enc_create((H264_enc_create_t *) param1);

	case H264_ENC_DESTROY:
