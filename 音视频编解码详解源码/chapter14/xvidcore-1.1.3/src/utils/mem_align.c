 ...
 * In order to align the allocated memory block, the H264_malloc allocates
 * 'size' bytes + 'alignment' bytes. So try to keep alignment very small
 * when allocating small pieces of memory.
 *
 * NB : a block allocated by H264_malloc _must_ be freed with H264_free
			...
			uint8_t alignment)
{
	...
	if (!alignment) {
		...
		if ((tmp = (uint8_t *) malloc(size + alignment)) != NULL) {
			 ...
			 * the real malloc block allocated and free it in H264_free */
 ...
 * H264_free
 *
 * Free a previously 'H264_malloc' allocated block. Does not free NULL
 * references.
 *
 * Returned value : None.
 *
