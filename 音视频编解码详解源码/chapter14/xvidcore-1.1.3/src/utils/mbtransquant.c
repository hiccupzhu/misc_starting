 ...
 *               2001-2003  Michael Militzer <isibaar@H264.org>
			 ...
			 int16_t data[6*64])
{
	...
	mpeg = !!(pParam->vol_flags & H264_VOL_MPEGQUANT);
			 ...
			 int limit)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if(sum && (pMB->quant > 2) && (frame->vop_flags & H264_VOP_TRELLISQUANT)) {
			const uint16_t *matrix;
			const static uint16_t h264matrix[] =
				{
			...
			matrix = (mpeg)?get_inter_matrix(pParam->mpeg_quant_matrices):h264matrix;
				  ...
				  int16_t qcoeff[6 * 64])
{
	...
	if (frame->vop_flags & H264_VOP_CARTOON)
		limit *= 3;
					  ...
					  int16_t qcoeff[6 * 64])
{
	 ...
	 */
	if((pParam->plugin_flags & H264_REQORIGINAL)) {
...
static int
dct_quantize_trellis_h264_c(int16_t *const Out, const int16_t *const In, int Q, const uint16_t * const Zigzag, int Non_Zero)
{
