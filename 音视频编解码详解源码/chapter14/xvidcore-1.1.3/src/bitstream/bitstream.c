 ...
 *
 *  Copyright (C) 2001-2003 Peter Ross <pross@H264.org>
						...
						int * intra_dc_threshold)
{
	...
	DPRINTF(H264_DEBUG_STARTCODE, "<video_packet_header>\n");
	...
	DPRINTF(H264_DEBUG_HEADER, "header_extension_code %i\n", hec);
	if (hec)
	{
		int time_base;
		int time_increment;
		...
		READ_MARKER();
		DPRINTF(H264_DEBUG_HEADER,"time %i:%i\n", time_base, time_increment);

		coding_type = BitstreamGetBits(bs, 2);
		DPRINTF(H264_DEBUG_HEADER,"coding_type %i\n", coding_type);
			...
			*intra_dc_threshold = intra_dc_threshold_table[BitstreamGetBits(bs, 3)];
			...
			if (coding_type != I_VOP && fcode_forward)
			{
				*fcode_forward = BitstreamGetBits(bs, 3);
				DPRINTF(H264_DEBUG_HEADER,"fcode_forward %i\n", *fcode_forward);
			}

			if (coding_type == B_VOP && fcode_backward)
			{
				*fcode_backward = BitstreamGetBits(bs, 3);
				DPRINTF(H264_DEBUG_HEADER,"fcode_backward %i\n", *fcode_backward);
			}
		}
	}
	...
	if (dec->newpred_enable)
	{
		...
		DPRINTF(H264_DEBUG_HEADER, "vop_id %i\n", vop_id);
		if (BitstreamGetBit(bs))	/* vop_id_for_prediction_indication */
		{
			...
			DPRINTF(H264_DEBUG_HEADER, "vop_id_for_prediction %i\n", vop_id_for_prediction);
		}
		READ_MARKER();
	}
...
read_vol_complexity_estimation_header(Bitstream * bs, DECODER * dec)
{
	...
	DPRINTF(H264_DEBUG_HEADER,"+ complexity_estimation_header; method=%i\n", e->method);
					 ...
					 WARPPOINTS *gmc_warp)
{
	...
	while ((BitstreamPos(bs) >> 3) + 4 <= bs->length) {
		...
		if (start_code == VISOBJSEQ_START_CODE) {
			...
			DPRINTF(H264_DEBUG_STARTCODE, "<visual_object>\n");
			...
			if (BitstreamGetBit(bs))	/* is_visual_object_identified */
			{
				...
				DPRINTF(H264_DEBUG_HEADER,"visobj_ver_id %i\n", dec->ver_id);
			...
			{
				DPRINTF(H264_DEBUG_ERROR, "visual_object_type != video\n");
				return -1;
			}
			BitstreamSkip(bs, 4);
			...
			DPRINTF(H264_DEBUG_STARTCODE, "<video_object>\n");
			DPRINTF(H264_DEBUG_HEADER, "vo id %i\n", start_code & VIDOBJ_START_CODE_MASK);

			BitstreamSkip(bs, 32);	/* video_object_start_code */

		} else
			//if ((start_code & ~VIDOBJLAY_START_CODE_MASK) == VIDOBJLAY_START_CODE)
		
			// luhanyu			
			//if ((start_code & ~VIDOBJLAY_START_CODE_MASK) == VIDOBJLAY_START_CODE) 	
			if (start_code == VIDOBJLAY_START_CODE)		{

			DPRINTF(H264_DEBUG_STARTCODE, "<video_object_layer>\n");
			DPRINTF(H264_DEBUG_HEADER, "vol id %i\n", start_code & VIDOBJLAY_START_CODE_MASK);
			...
			{
				DPRINTF(H264_DEBUG_HEADER, "+ is_object_layer_identifier\n");
				vol_ver_id = BitstreamGetBits(bs, 4);	/* video_object_layer_verid */
				DPRINTF(H264_DEBUG_HEADER,"ver_id %i\n", vol_ver_id);
			...
			{
				DPRINTF(H264_DEBUG_HEADER, "+ aspect_ratio_info\n");
			...
			{
				DPRINTF(H264_DEBUG_HEADER, "+ vol_control_parameters\n");
				BitstreamSkip(bs, 2);	/* chroma_format */
				dec->low_delay = BitstreamGetBit(bs);	/* low_delay */
				DPRINTF(H264_DEBUG_HEADER, "low_delay %i\n", dec->low_delay);
				if (BitstreamGetBit(bs))	/* vbv_parameters */
				{
					...
					DPRINTF(H264_DEBUG_HEADER,"+ vbv_parameters\n");
					...
					DPRINTF(H264_DEBUG_HEADER,"bitrate %d (unit=400 bps)\n", bitrate);
					DPRINTF(H264_DEBUG_HEADER,"buffer_size %d (unit=16384 bits)\n", buffer_size);
					DPRINTF(H264_DEBUG_HEADER,"occupancy %d (unit=64 bits)\n", occupancy);
				}
			}else{
			...
			DPRINTF(H264_DEBUG_HEADER, "shape %i\n", dec->shape);
			if (dec->shape != VIDOBJLAY_SHAPE_RECTANGULAR)
			{
				DPRINTF(H264_DEBUG_ERROR,"non-rectangular shapes are not supported\n");
			}
			...
			{
				DPRINTF(H264_DEBUG_HEADER, "+ fixed_vop_rate\n");
			...
			if (dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) {
				...
				if (dec->shape == VIDOBJLAY_SHAPE_RECTANGULAR) {
					...
					DPRINTF(H264_DEBUG_HEADER, "width %i\n", width);
					DPRINTF(H264_DEBUG_HEADER, "height %i\n", height);

					if (dec->width != width || dec->height != height)
					{
						if (dec->fixed_dimensions)
						{
							DPRINTF(H264_DEBUG_ERROR, "decoder width/height does not match bitstream\n");
							return -1;
						}
						resize = 1;
						dec->width = width;
				...
				DPRINTF(H264_DEBUG_HEADER, "interlacing %i\n", dec->interlacing);

				if (!BitstreamGetBit(bs))	/* obmc_disable */
				{
					DPRINTF(H264_DEBUG_ERROR, "obmc_disabled==false not supported\n");
					/* TODO */
				...
				{
					DPRINTF(H264_DEBUG_HEADER, "not_8_bit==true (ignored)\n");
				...
				DPRINTF(H264_DEBUG_HEADER, "quant_type %i\n", dec->quant_type);
				...
				if (dec->quant_type) {
					if (BitstreamGetBit(bs))	/* load_intra_quant_mat */
					{
						...
						DPRINTF(H264_DEBUG_HEADER, "load_intra_quant_mat\n");
					...
					if (BitstreamGetBit(bs))	/* load_inter_quant_mat */
					{
						...
						DPRINTF(H264_DEBUG_HEADER, "load_inter_quant_mat\n");
					...
					if (dec->shape == VIDOBJLAY_SHAPE_GRAYSCALE) {
						DPRINTF(H264_DEBUG_ERROR, "greyscale matrix not supported\n");
						return -1;
					}
				...
				if (vol_ver_id != 1) {
					...
					DPRINTF(H264_DEBUG_HEADER,"quarterpel %i\n", dec->quarterpel);
				}
				else
					dec->quarterpel = 0;
				...
				{
					DPRINTF(H264_DEBUG_ERROR, "data_partitioned not supported\n");
				...
				if (vol_ver_id != 1) {
					...
					{
						DPRINTF(H264_DEBUG_HEADER, "+ newpred_enable\n");
					...
					DPRINTF(H264_DEBUG_HEADER, "reduced_resolution_enable %i\n", dec->reduced_resolution_enable);
				}
				else
				{
				...
				if (dec->scalability)
				{
					DPRINTF(H264_DEBUG_ERROR, "scalability not supported\n");
			...
			DPRINTF(H264_DEBUG_STARTCODE, "<group_of_vop>\n");
			...
			BitstreamSkip(bs, 32);
			{
				...
				DPRINTF(H264_DEBUG_HEADER, "time %ih%im%is\n", hours,minutes,seconds);
			}
			...
			DPRINTF(H264_DEBUG_STARTCODE, "<vop>\n");
			...
			DPRINTF(H264_DEBUG_HEADER, "time_base %i\n", time_incr);
			DPRINTF(H264_DEBUG_HEADER, "time_increment %i\n", time_increment);

			DPRINTF(H264_DEBUG_TIMECODE, "%c %i:%i\n",
			...
			DPRINTF(H264_DEBUG_HEADER,"time_pp=%i\n", dec->time_pp);
			DPRINTF(H264_DEBUG_HEADER,"time_bp=%i\n", dec->time_bp);

			READ_MARKER();

			if (!BitstreamGetBit(bs))	/* vop_coded */
			{
				DPRINTF(H264_DEBUG_HEADER, "vop_coded==false\n");
				return N_VOP;
			}
			...
			if ((dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) &&
				...
				*rounding = BitstreamGetBit(bs);	/* rounding_type */
				DPRINTF(H264_DEBUG_HEADER, "rounding %i\n", *rounding);
			}
			...
			if (dec->reduced_resolution_enable &&
				...
				if (BitstreamGetBit(bs));
					DPRINTF(H264_DEBUG_ERROR, "RRV not supported (anymore)\n");
			}
			...
			if (dec->shape != VIDOBJLAY_SHAPE_RECTANGULAR) {
				if(!(dec->sprite_enable == SPRITE_STATIC && coding_type == I_VOP)) {
					...
					DPRINTF(H264_DEBUG_HEADER, "width %i\n", width);
					DPRINTF(H264_DEBUG_HEADER, "height %i\n", height);
					DPRINTF(H264_DEBUG_HEADER, "horiz_mc_ref %i\n", horiz_mc_ref);
					DPRINTF(H264_DEBUG_HEADER, "vert_mc_ref %i\n", vert_mc_ref);
				}
			...
			if (dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) {
				...
				*intra_dc_threshold =
				...
				if (dec->interlacing) {
					...
					DPRINTF(H264_DEBUG_HEADER, "interlace top_field_first %i\n", dec->top_field_first);
					dec->alternate_vertical_scan = BitstreamGetBit(bs);
					DPRINTF(H264_DEBUG_HEADER, "interlace alternate_vertical_scan %i\n", dec->alternate_vertical_scan);
			...
			if ((dec->sprite_enable == SPRITE_STATIC || dec->sprite_enable== SPRITE_GMC) && coding_type == S_VOP) {
				...
				for (i = 0 ; i < dec->sprite_warping_points; i++)
				{
					...
					DPRINTF(H264_DEBUG_HEADER,"sprite_warping_point[%i] xy=(%i,%i)\n", i, x, y);
				}
			...
			DPRINTF(H264_DEBUG_STARTCODE, "<user_data>: %s\n", tmp);

			/* read H264 bitstream version */
			if(strncmp(tmp, "H264", 4) == 0) {
				if (tmp[strlen(tmp)-1] == 'C') {				
					sscanf(tmp, "H264%dC", &dec->bs_version);
					dec->cartoon_mode = 1;
				}
				else
					sscanf(tmp, "H264%d", &dec->bs_version);

				DPRINTF(H264_DEBUG_HEADER, "H264 bitstream version=%i\n", dec->bs_version);
			}
			...
			if (i >= 2)
			{
				...
				DPRINTF(H264_DEBUG_HEADER, "divx version=%i, build=%i packed=%i\n",
		...
		} else					/* start_code == ? */
		{
			if (BitstreamShowBits(bs, 24) == 0x000001) {
				DPRINTF(H264_DEBUG_STARTCODE, "<unknown: %x>\n", BitstreamShowBits(bs, 32));
			}
			BitstreamSkip(bs, 8);
		}
	}
						...
						const FRAMEINFO * const frame)
{
	...
	if ( (pParam->vol_flags & H264_VOL_QUARTERPEL) ||
         (pParam->vol_flags & H264_VOL_GMC))
		vol_ver_id = 2;

    if ((pParam->vol_flags & (H264_VOL_MPEGQUANT|H264_VOL_QUARTERPEL|H264_VOL_GMC|H264_VOL_INTERLACING)) ||
...
//	BitstreamPutBits(bs, VIDOBJ_START_CODE|(vo_id&0x5), 32);
	...
	BitstreamPutBit(bs, pParam->vol_flags & H264_VOL_INTERLACING);	/* interlace */
	BitstreamPutBit(bs, 1);		/* obmc_disable (overlapped block motion compensation) */

	if (vol_ver_id != 1)
	{	if ((pParam->vol_flags & H264_VOL_GMC))
	...
	BitstreamPutBit(bs, pParam->vol_flags & H264_VOL_MPEGQUANT);

	if ((pParam->vol_flags & H264_VOL_MPEGQUANT)) {
...
#define DIVX5_ID ((char *)"DivX503b1393")
  if ((pParam->global_flags & H264_GLOBAL_DIVX5_USERDATA)) {
    BitstreamWriteUserData(bs, DIVX5_ID, strlen(DIVX5_ID));
  	if (pParam->max_bframes > 0 && (pParam->global_flags & H264_GLOBAL_PACKED))
      BitstreamPutBits(bs, 'p', 8);
	}

#if 0 // luhanyu del this
	/* H264 id */
	{
		const char H264_user_format[] = "H264%04d%c";
		char H264_user_data[100];
		sprintf(H264_user_data,
				H264_user_format,
				H264_BS_VERSION,
				(frame->vop_flags & H264_VOP_CARTOON)?'C':'\0');
		BitstreamWriteUserData(bs, H264_user_data, strlen(H264_user_data));
	}
#endif
						...
						unsigned int quant)
{
	...
	if ((frame->vol_flags & H264_VOL_INTERLACING)) {
		BitstreamPutBit(bs, (frame->vop_flags & H264_VOP_TOPFIELDFIRST));
		BitstreamPutBit(bs, (frame->vop_flags & H264_VOP_ALTERNATESCAN));
	}
	...
	if (frame->coding_type == S_VOP) {
		if (1)	{		/* no_of_sprite_warping_points>=1 (we use 2!) */
			...
			if ((frame->vol_flags & H264_VOL_QUARTERPEL))
			{
				DPRINTF(H264_DEBUG_HEADER,"sprite_warping_point[%i] xy=(%i,%i) *QPEL*\n", k, frame->warp.duv[k].x/2, frame->warp.duv[k].y/2);
			}
			else
			{
