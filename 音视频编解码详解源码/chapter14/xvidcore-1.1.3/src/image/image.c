...
#include <math.h>
#include "../portab.h"
#include "../global.h"			/* H264_CSP_XXX's */
#include "../H264.h"			/* H264_CSP_XXX's */
#include "image.h"
			 ...
			 uint32_t edged_height)
{
	...
	image->y =
		H264_malloc(edged_width * (edged_height + 1) + SAFETY, CACHE_LINE);
	if (image->y == NULL) {
		return -1;
	}
	memset(image->y, 0, edged_width * (edged_height + 1) + SAFETY);

	image->u = H264_malloc(edged_width2 * edged_height2 + SAFETY, CACHE_LINE);
	if (image->u == NULL) {
		H264_free(image->y);
		image->y = NULL;
		return -1;
	...
	image->v = H264_malloc(edged_width2 * edged_height2 + SAFETY, CACHE_LINE);
	if (image->v == NULL) {
		H264_free(image->u);
		image->u = NULL;
		H264_free(image->y);
			  ...
			  uint32_t edged_height)
{
	...
	if (image->y) {
		H264_free(image->y - (EDGE_SIZE * edged_width + EDGE_SIZE));
		image->y = NULL;
	}
	if (image->u) {
		H264_free(image->u - (EDGE_SIZE2 * edged_width2 + EDGE_SIZE2));
		image->u = NULL;
	}
	if (image->v) {
		H264_free(image->v - (EDGE_SIZE2 * edged_width2 + EDGE_SIZE2));
		image->v = NULL;
	}
}
			   ...
			   int bs_version)
{
	 ...
	 * pixel width and height multiples. This was not respected in old H264s */
...
#undef IMG_V
	...
	DPRINTF(H264_DEBUG_DEBUG,"chroma_optimized_pixels = %i/%i\n", pixels, width*height/4);
}
...
unsigned char local_data[64*16*3/2];
#endif
...
void
output_slice(IMAGE * cur, int stride, int width, H264_image_t* out_frm, int mbx, int mby,int mbl) {
