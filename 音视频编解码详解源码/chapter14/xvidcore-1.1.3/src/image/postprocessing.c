 ...
 *
 *  Copyright(C) 2003-2004 Michael Militzer <isibaar@H264.org>
...
void init_postproc(H264_POSTPROC *tbls)
{
	init_deblock(tbls);
	init_noise(tbls);
}
...
void
image_postproc(H264_POSTPROC *tbls, IMAGE * img, int edged_width,
				...
				int flags, int brightness, int frame_num, int bvop)
{
	...
	int i,j;
	int quant;
	int dering = flags & H264_DERINGY;

	/* luma: j,i in block units */
	if ((flags & H264_DEBLOCKY))
	{
	...
	/* chroma */
	if ((flags & H264_DEBLOCKUV))
	{
		dering = flags & H264_DERINGUV;
...
void init_deblock(H264_POSTPROC *tbls)
{
	int i;

	for(i = -255; i < 256; i++) {
		tbls->H264_thresh_tbl[i + 255] = 0;
		if(ABS(i) < THR1)
			tbls->H264_thresh_tbl[i + 255] = 1;
		tbls->H264_abs_tbl[i + 255] = ABS(i);
	}
}
...
#define APPLY_FILTER_CORE \
		...
		eq_cnt = 0; \
		\
		eq_cnt += tbls->H264_thresh_tbl[s[0] - s[1] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[1] - s[2] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[2] - s[3] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[3] - s[4] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[4] - s[5] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[5] - s[6] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[6] - s[7] + 255]; \
		eq_cnt += tbls->H264_thresh_tbl[s[7] - s[8] + 255]; \
		\
		if(eq_cnt < THR2) { /* Default mode */  \
			int a30, a31, a32;					\
			int diff, limit;					\
												\
			if(tbls->H264_abs_tbl[(s[4] - s[5]) + 255] < quant) {			\
			...
			if (dering) {	\
				e[0] = (tbls->H264_abs_tbl[(s[0] - s[1]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[1] = (tbls->H264_abs_tbl[(s[1] - s[2]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[2] = (tbls->H264_abs_tbl[(s[2] - s[3]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[3] = (tbls->H264_abs_tbl[(s[3] - s[4]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[4] = (tbls->H264_abs_tbl[(s[4] - s[5]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[5] = (tbls->H264_abs_tbl[(s[5] - s[6]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[6] = (tbls->H264_abs_tbl[(s[6] - s[7]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[7] = (tbls->H264_abs_tbl[(s[7] - s[8]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				e[8] = (tbls->H264_abs_tbl[(s[8] - s[9]) + 255] > quant + DERING_STRENGTH) ? 1 : 0;	\
				\
				e[1] |= e[0];	\
				e[2] |= e[1];	\
				e[3] |= e[2];	\
			...
			if(((max-min)) < 2*quant) {	\
										\
				/* Choose edge pixels */	\
				p0 = (tbls->H264_abs_tbl[(s[1] - s[0]) + 255] < quant) ? s[0] : s[1];	\
				p9 = (tbls->H264_abs_tbl[(s[8] - s[9]) + 255] < quant) ? s[9] : s[8];	\
																\
...
void deblock8x8_h(H264_POSTPROC *tbls, uint8_t *img, int stride, int quant, int dering)
{
	int eq_cnt;
	uint8_t *v[10];
	int s[10];
...
void deblock8x8_v(H264_POSTPROC *tbls, uint8_t *img, int stride, int quant, int dering)
{
	int eq_cnt;
	uint8_t *v[10];
	int s[10];
...
void init_noise(H264_POSTPROC *tbls)
{
	int i, j;
	int patt[4] = { -1,0,1,0 };
	...
	for(i = 0, j = 0; i < MAX_NOISE; i++, j++)
	{
		...
		y1 /= 3.0;
		y2 /= 3.0;
		tbls->H264_noise1[i] = (int) y1;
		tbls->H264_noise2[i] = (int) y2;
	...
	for (i = 0; i < MAX_RES; i++)
		for (j = 0; j < 3; j++) {
			tbls->H264_prev_shift[i][j] = tbls->H264_noise1 + (rand() & (MAX_SHIFT - 1));
			tbls->H264_prev_shift[i][3 + j] = tbls->H264_noise2 + (rand() & (MAX_SHIFT - 1));
		}
}

void add_noise(H264_POSTPROC *tbls, uint8_t *dst, uint8_t *src, int stride, int width, int height, int shiftptr, int quant)
{
	int x, y;
	int shift = 0;
	int add = (quant < 5) ? 3 : 0;
	int8_t *noise = (quant < 5) ? tbls->H264_noise2 : tbls->H264_noise1;
	...
	for(y = 0; y < height; y++)
	{
		...
		for(x = 0; x < width; x++)
		{
			const int n = tbls->H264_prev_shift[y][0 + add][x] + tbls->H264_prev_shift[y][1 + add][x] + 
				          tbls->H264_prev_shift[y][2 + add][x];

			dst[x] = src2[x] + ((n * src2[x]) >> 7);
		}

		tbls->H264_prev_shift[y][shiftptr + add] = noise + shift;
