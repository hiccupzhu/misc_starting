 ...
 *               2002-2004 Peter Ross <pross@H264.org>
 *
...
decoder_resize(DECODER * dec)
{
  ...
  H264_free(dec->last_mbs);
  H264_free(dec->mbs);
  H264_free(dec->qscale);
  dec->last_mbs = NULL;
  dec->mbs = NULL;
  dec->qscale = NULL;
	...
	dec->mbs =
		H264_malloc(sizeof(MACROBLOCK) * dec->mb_width * dec->mb_height,
					CACHE_LINE);
	if (dec->mbs == NULL)
	  goto memory_error;
	...
	dec->qscale =
		H264_malloc(sizeof(int) * dec->mb_width * dec->mb_height, CACHE_LINE);
...
int
decoder_create(H264_dec_create_t * create)
{
  DECODER *dec;
  ...
  dec = H264_malloc(sizeof(DECODER), CACHE_LINE);
  if (dec == NULL) {
    return H264_ERR_MEMORY;
  }
  ...
  dec->mpeg_quant_matrices = H264_malloc(sizeof(uint16_t) * 64 * 8, CACHE_LINE);
...
decoder_destroy(DECODER * dec)
{
  ...
  image_destroy(&dec->qtmp, dec->edged_width, dec->edged_height);
  image_destroy(&dec->cur, dec->edged_width, dec->edged_height);
  H264_free(dec->mpeg_quant_matrices);
  H264_free(dec);
        ...
        const unsigned int bound)
{
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (quant < intra_dc_threshold) {
      ...
      DPRINTF(H264_DEBUG_COEFF,"block[0] %i\n", dc_dif);
    } else {
      start_coeff = 0;
    }
    ...
    start_timer();
    if (dec->quant_type == 0) {
      dequant_h264_intra(&data[i * 64], &block[i * 64], iQuant, iDcScaler, dec->mpeg_quant_matrices);
    } else {
...
#define CHECK_MV(mv) \
  ...
  if ((mv).x > xborder_high) { \
    DPRINTF(H264_DEBUG_MV, "mv.x > max -- %d > %d, MB %d, %d", (mv).x, xborder_high, x_pos, y_pos); \
    (mv).x = xborder_high; \
  } else if ((mv).x < xborder_low) { \
    DPRINTF(H264_DEBUG_MV, "mv.x < min -- %d < %d, MB %d, %d", (mv).x, xborder_low, x_pos, y_pos); \
    (mv).x = xborder_low; \
  } \
  if ((mv).y > yborder_high) { \
    DPRINTF(H264_DEBUG_MV, "mv.y > max -- %d > %d, MB %d, %d", (mv).y, yborder_high, x_pos, y_pos); \
    (mv).y = yborder_high; \
  } else if ((mv).y < yborder_low) { \
    DPRINTF(H264_DEBUG_MV, "mv.y < min -- %d < %d, MB %d, %d", (mv).y, yborder_low, x_pos, y_pos); \
    (mv).y = yborder_low; \
  } \
  } while (0)
        ...
        int intra_dc_threshold)
{
  ...
  for (y = 0; y < mb_height; y++) {
    for (x = 0; x < mb_width; x++) {
      ...
      DPRINTF(H264_DEBUG_MB, "macroblock (%i,%i) %08x\n", x, y, BitstreamShowBits(bs, 32));
      ...
      if (dec->interlacing) {
        ...
        DPRINTF(H264_DEBUG_MB,"deci: field_dct: %i\n", mb->field_dct);
      }
        ...
        const int bound)
{
  ...
  DPRINTF(H264_DEBUG_MV,"mv_diff (%i,%i) pred (%i,%i) result (%i,%i)\n", mv.x, mv.y, pmv.x, pmv.y, mv.x+pmv.x, mv.y+pmv.y);
  ...
  for (y = 0; y < mb_height; y++) {
    ...
    for (x = 0; x < mb_width; x++) {
      ...
      if (!(BitstreamGetBit(bs))) { /* block _is_ coded */
        ...
        DPRINTF(H264_DEBUG_MB, "mode %i\n", mb->mode);
        DPRINTF(H264_DEBUG_MB, "cbpc %i\n", cbpc);
        ...
        DPRINTF(H264_DEBUG_MB, "cbpy %i mcsel %i \n", cbpy,mcsel);

        cbp = (cbpy << 2) | cbpc;

        if (mb->mode == MODE_INTER_Q || mb->mode == MODE_INTRA_Q) {
          int dquant = dquant_table[BitstreamGetBits(bs, 2)];
          DPRINTF(H264_DEBUG_MB, "dquant %i\n", dquant);
          quant += dquant;
          if (quant > 31) {
            quant = 31;
          } else if (quant < 1) {
        ...
        if (dec->interlacing) {
          if (cbp || intra) {
            mb->field_dct = BitstreamGetBit(bs);
            DPRINTF(H264_DEBUG_MB,"decp: field_dct: %i\n", mb->field_dct);
          }

          if ((mb->mode == MODE_INTER || mb->mode == MODE_INTER_Q) && !mcsel) {
            mb->field_pred = BitstreamGetBit(bs);
            DPRINTF(H264_DEBUG_MB, "decp: field_pred: %i\n", mb->field_pred);

            if (mb->field_pred) {
              mb->field_for_top = BitstreamGetBit(bs);
              DPRINTF(H264_DEBUG_MB,"decp: field_for_top: %i\n", mb->field_for_top);
              mb->field_for_bot = BitstreamGetBit(bs);
              DPRINTF(H264_DEBUG_MB,"decp: field_for_bot: %i\n", mb->field_for_bot);
            }
          }
        }
        ...
        int fcode_backward)
{
  ...
  for (y = 0; y < dec->mb_height; y++) {
    ...
    for (x = 0; x < dec->mb_width; x++) {
      ...
      default:
        DPRINTF(H264_DEBUG_ERROR,"Not supported B-frame mb_type = %i\n", mb->mode);
      }
    } /* End of for */
  }
}
/*为了版权保护，在待编码的图像上增加logo*/
#define swust 1
/*~为了版权保护，在待编码的图像上增加logo*/

#if swust
//#include "logodata.h"
extern unsigned char logo_data[64*16*3/2];

#endif

/* perform post processing if necessary, and output the image */
static void decoder_output(DECODER * dec, IMAGE * img, MACROBLOCK * mbs,
          H264_dec_frame_t * frame, H264_dec_stats_t * stats,
          int coding_type, int quant)
{
	const int brightness = H264_VERSION_MINOR(frame->version) >= 1 ? frame->brightness : 0;
	const edged_width = dec->edged_width;
	const edged_width2 = dec->edged_width/2;
	const width = dec->width;
	uint8_t *p_logo;
	uint32_t i;

  if (dec->cartoon_mode)
    frame->general &= ~H264_FILMEFFECT;

  if ((frame->general & (H264_DEBLOCKY|H264_DEBLOCKUV|H264_FILMEFFECT) || brightness!=0)
  ...
  image_output(img, dec->width, dec->height,
	...
	for (i=0; i<16; i++)
	{
		memcpy(img->y + width - 64-8+4*edged_width+i*edged_width, p_logo+i*64, 64);
		memcpy(img->y + width - 64-8+4*edged_width+(i+16)*edged_width, p_logo+i*64, 64);

	}
	for (i=0; i< 8; i++)
	{
		memcpy(img->u+width/2-32-4+2*edged_width2+i*edged_width2,p_logo+64*16+i*32, 32);
		memcpy(img->u+width/2-32-4+2*edged_width2+(i+8)*edged_width2,p_logo+64*16+i*32, 32);
		memcpy(img->v+width/2-32-4+2*edged_width2+i*edged_width2,p_logo+64*16*5/4+i*32,32);
		memcpy(img->v+width/2-32-4+2*edged_width2+(i+8)*edged_width2,p_logo+64*16*5/4+i*32,32);
	}
	//DAT_wait(idy);
	//DAT_wait(idu);
	//DAT_wait(idv);
        ...
        H264_dec_frame_t * frame, H264_dec_stats_t * stats)
{
  ...
  if (H264_VERSION_MAJOR(frame->version) != 1 || (stats && H264_VERSION_MAJOR(stats->version) != 1))  /* v1.x.x */
    return H264_ERR_VERSION;

  start_global_timer();

  dec->low_delay_default = (frame->general & H264_LOWDELAY);
  if ((frame->general & H264_DISCONTINUITY))
    dec->frames = 0;
  dec->out_frm = (frame->output.csp == H264_CSP_SLICE) ? &frame->output : NULL;
  ...
  if(frame->length<0) {  /* decoder flush */
    ...
    if (!(dec->low_delay_default && dec->packed_mode) && !dec->low_delay && dec->frames>0) {
      ...
      if (stats) stats->type = H264_TYPE_NOTHING;
      ret = H264_ERR_END;
    }
  ...
  /* XXX: 0x7f is only valid whilst decoding vfw H264/divx5 avi's */
...
repeat:
  ...
  DPRINTF(H264_DEBUG_HEADER, "coding_type=%i,  packed=%i,  time=%"
#if defined(_MSC_VER)
    "I64"
#else
    "ll"
  ...
  if (dec->packed_mode && coding_type == N_VOP) {
    ...
    if (dec->low_delay) {
      DPRINTF(H264_DEBUG_ERROR, "warning: bvop found in low_delay==1 stream\n");
      dec->low_delay = 0;
    }
