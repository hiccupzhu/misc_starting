 ...
 *  - Prediction module -
 *
 *  Copyright (C) 2001-2003 Michael Militzer <isibaar@H264.org>
 *                2001-2003 Peter Ross <pross@H264.org>
 *
		 ...
		 const int bsversion)
{
	...
	DPRINTF(H264_DEBUG_COEFF,"predictor[0] %i\n", predictors[0]);
	...
	if (acpred_direction == 1) {
		for (i = 1; i < 8; i++) {
			...
			DPRINTF(H264_DEBUG_COEFF,"predictor[%i] %i\n",i, predictors[i]);
			 ...
			 int16_t qcoeff[6 * 64])
{
	...
	if ((pMB->mode == MODE_INTRA) || (pMB->mode == MODE_INTRA_Q)) {
		...
		for (j = 0; j < 6; j++) {
			...
			if ((frame->vop_flags & H264_VOP_HQACPRED))
