...
static int
rc_single_create(H264_plg_create_t * create,
			  rc_single_t ** handle)
{
	H264_plugin_single_t *param = (H264_plugin_single_t *) create->param;
	rc_single_t *rc;
	int i;
	...
	*handle = rc;
	...
	/* A bit of debug info */
	DPRINTF(H264_DEBUG_RC, "bytes_per_sec: %i\n", rc->bytes_per_sec);
	DPRINTF(H264_DEBUG_RC, "frame rate   : %f\n", (double) create->fbase / create->fincr);
	DPRINTF(H264_DEBUG_RC, "target_framesize: %f\n", rc->target_framesize);
	 ...
	 if (data->quant <= 0) {
		if (data->zone && data->zone->mode == H264_ZONE_QUANT) {
			...
			/* limit to min/max range 
			   ...
			   P-VOP's range... */
			if (q > data->max_quant[H264_TYPE_PVOP-1])
				q = data->max_quant[H264_TYPE_PVOP-1];
			else if (q < data->min_quant[H264_TYPE_PVOP-1])
				q = data->min_quant[H264_TYPE_PVOP-1];
...
int
H264_plugin_single(void *handle,
				int opt,
				void *param1,
				void *param2)
{
	...
	case H264_PLG_CREATE:
		return rc_single_create((H264_plg_create_t *) param1, param2);

	case H264_PLG_DESTROY:
		return rc_single_destroy((rc_single_t *) handle,(H264_plg_destroy_t *) param1);

	case H264_PLG_BEFORE:
		return rc_single_before((rc_single_t *) handle, (H264_plg_data_t *) param1);

	case H264_PLG_AFTER:
		return rc_single_after((rc_single_t *) handle, (H264_plg_data_t *) param1);
	}
