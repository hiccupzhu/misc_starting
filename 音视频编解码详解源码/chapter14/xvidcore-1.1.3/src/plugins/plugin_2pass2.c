...
typedef struct
{
	...
	/* Total length used by H264_ZONE_QUANT zones */
	uint64_t tot_quant;
...
static int rc_2pass2_create(H264_plg_create_t * create, rc_2pass2_t ** handle);
static int rc_2pass2_before(rc_2pass2_t * rc, H264_plg_data_t * data);
static int rc_2pass2_after(rc_2pass2_t * rc, H264_plg_data_t * data);
static int rc_2pass2_destroy(rc_2pass2_t * rc, H264_plg_destroy_t * destroy);
...
int
H264_plugin_2pass2(void * handle, int opt, void * param1, void * param2)
{
	switch(opt) {
	case H264_PLG_INFO :
	case H264_PLG_FRAME :
	...
	case H264_PLG_CREATE :
		return rc_2pass2_create((H264_plg_create_t*)param1, param2);

	case H264_PLG_DESTROY :
		return rc_2pass2_destroy((rc_2pass2_t*)handle, (H264_plg_destroy_t*)param1);

	case H264_PLG_BEFORE :
		return rc_2pass2_before((rc_2pass2_t*)handle, (H264_plg_data_t*)param1);

	case H264_PLG_AFTER :
		return rc_2pass2_after((rc_2pass2_t*)handle, (H264_plg_data_t*)param1);
	}
...
static void zone_process(rc_2pass2_t *rc, const H264_plg_create_t * create);
...
static int
rc_2pass2_create(H264_plg_create_t * create, rc_2pass2_t **handle)
{
	H264_plugin_2pass2_t * param = (H264_plugin_2pass2_t *)create->param;
	rc_2pass2_t * rc;
	int i;
	...
	if (H264_VERSION_MINOR(param->version) >= 1) {
...
#undef _INIT
	...
	if (statsfile_count_frames(rc, param->filename) == -1) {
		DPRINTF(H264_DEBUG_RC,"[H264 rc] -- ERROR: fopen %s failed\n", param->filename);
		free(rc);
		return(H264_ERR_FAIL);
	}
	...
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Frame rate: %d/%d (%ffps)\n",
			create->fbase, create->fincr,
			(double)create->fbase/(double)create->fincr);
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Number of frames: %d\n", rc->num_frames);
	if(rc->param.bitrate>=0)
		DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Target bitrate: %ld\n", rc->param.bitrate);
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Target filesize: %lld\n", rc->target);

	/* Compensate the average frame overhead caused by the container */
	rc->target -= rc->num_frames*rc->param.container_frame_overhead;
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Container Frame overhead: %d\n", rc->param.container_frame_overhead);
	if(rc->param.container_frame_overhead)
		DPRINTF(H264_DEBUG_RC, "[H264 rc] -- New target filesize after container compensation: %lld\n", rc->target);
	...
	if (rc->param.bitrate) {
		...
		for (i=0;i<rc->num_frames;i++) {
			rc->stats[i].zone_mode = H264_ZONE_WEIGHT;
	...
	if (rc->param.vbv_size > 0)  {
		...
		if (status) {
			DPRINTF(H264_DEBUG_RC, "[H264 rc] Underflow detected - Scaling Curve for compliancy.\n");
		}

		status = scale_curve_for_vbv_compliancy(rc, fps);

		if (status == 0) {
			DPRINTF(H264_DEBUG_RC, "[H264 rc] VBV compliant curve scaling done.\n");
		} else {
			DPRINTF(H264_DEBUG_RC, "[H264 rc] VBV compliant curve scaling impossible.\n");
		}
	}
	*handle = rc;
	return(0);
...
static int
rc_2pass2_destroy(rc_2pass2_t * rc, H264_plg_destroy_t * destroy)
{
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- target_total:%lld desired_total:%.2f (%.2f%%) actual_total:%.2f (%.2f%%)\n",
			rc->target,
			rc->desired_total,
...
static int
rc_2pass2_before(rc_2pass2_t * rc, H264_plg_data_t * data)
{
	...
	if (data->frame_num >= rc->num_frames) {
		DPRINTF(H264_DEBUG_RC,"[H264 rc] -- stats file too short (now processing frame %d)",
			data->frame_num);
		return(0);
	}
	...
	if (s->zone_mode == H264_ZONE_QUANT) {
		/* Quant stuff */
	...
	if (s->type != H264_TYPE_IVOP || rc->overflow > 0) {
		 ...
		 * overall size */
		frametype_factor  = rc->count[H264_TYPE_IVOP-1]*rc->avg_length[H264_TYPE_IVOP-1];
		frametype_factor += rc->count[H264_TYPE_PVOP-1]*rc->avg_length[H264_TYPE_PVOP-1];
		frametype_factor += rc->count[H264_TYPE_BVOP-1]*rc->avg_length[H264_TYPE_BVOP-1];
	...
	if(s->type == H264_TYPE_BVOP) {

		twopass_stat_t *b_ref = s;

		/* Find the reference frame */
		while(b_ref != &rc->stats[0] && b_ref->type == H264_TYPE_BVOP)
			b_ref--;
	...
	/* We know H264core will apply the bframe formula again, so we compensate
	...
	if (s->type != H264_TYPE_IVOP && rc->last_quant[s->type-1] && capped_to_max_framesize == 0) {

		if (data->quant > rc->last_quant[s->type-1] + 2) {
			data->quant = rc->last_quant[s->type-1] + 2;
			DPRINTF(H264_DEBUG_RC,
					"[H264 rc] -- frame %d p/b-frame quantizer prevented from rising too steeply\n",
					data->frame_num);
		}
		if (data->quant < rc->last_quant[s->type-1] - 2) {
			data->quant = rc->last_quant[s->type-1] - 2;
			DPRINTF(H264_DEBUG_RC,
					"[H264 rc] -- frame:%d p/b-frame quantizer prevented from falling too steeply\n",
					data->frame_num);
		}
	}
...
static int
rc_2pass2_after(rc_2pass2_t * rc, H264_plg_data_t * data)
{
	...
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- frame:%d type:%c quant:%d stats:%d scaled:%d desired:%d actual:%d error:%d overflow:%.2f\n",
			data->frame_num,
...
statsfile_count_frames(rc_2pass2_t * rc, char * filename)
{
	...
	while ((line = readline(f)) != NULL) {
		...
		if (fields == 1) {
			...
			default:
				DPRINTF(H264_DEBUG_RC,
						"[H264 rc] -- WARNING: L%d unknown frame type used (%c).\n",
						lines, type);
			}
		} else {
				DPRINTF(H264_DEBUG_RC,
						"[H264 rc] -- WARNING: L%d misses some stat fields (%d).\n",
						lines, 7-fields);
		}
...
statsfile_load(rc_2pass2_t *rc, char * filename)
{
	...
	while(processed_entries < rc->num_frames) {
		...
		case 's':
		case 'S':
			s->type = H264_TYPE_PVOP;
			s->invariant /= INVARIANT_HEADER_PART_PVOP;
			break;
		case 'b':
		case 'B':
			s->type = H264_TYPE_BVOP;
...
first_pass_stats_prepare_data(rc_2pass2_t * rc)
{
	...
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Min 1st pass IFrame length: %d\n", rc->min_length[0]);
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Min 1st pass PFrame length: %d\n", rc->min_length[1]);
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Min 1st pass BFrame length: %d\n", rc->min_length[2]);
}

/* calculate zone weight "center" */
static void
zone_process(rc_2pass2_t *rc, const H264_plg_create_t * create)
{
	int i,j;
	int n = 0;
	...
	if (create->num_zones == 0) {
		for (j = 0; j < rc->num_frames; j++) {
			rc->stats[j].zone_mode = H264_ZONE_WEIGHT;
	...
	for(i=0; i < create->num_zones; i++) {
		...
		if (create->zones[i].mode == H264_ZONE_WEIGHT) {
			...
			n += next;
		} else{  /* H264_ZONE_QUANT */
			for (j = create->zones[i].frame; j < next && j < rc->num_frames; j++ ) {
				rc->stats[j].zone_mode = H264_ZONE_QUANT;
				rc->stats[j].weight = (double)create->zones[i].increment / (double)create->zones[i].base;
...
first_pass_scale_curve_internal(rc_2pass2_t *rc)
{
	...
	total_invariant	 = rc->tot_invariant[H264_TYPE_IVOP-1];
	total_invariant += rc->tot_invariant[H264_TYPE_PVOP-1];
	total_invariant += rc->tot_invariant[H264_TYPE_BVOP-1];
	...
	if(rc->min_length[H264_TYPE_IVOP-1] > ((num_MBs*22) + 240) / 8)
		rc->min_length[H264_TYPE_IVOP-1] = ((num_MBs*22) + 240) / 8;

	/* Minimum for P/S frames */
	if(rc->min_length[H264_TYPE_PVOP-1] > ((num_MBs) + 88)  / 8)
		rc->min_length[H264_TYPE_PVOP-1] = ((num_MBs) + 88)  / 8;

	/* Minimum for B frames */
	if(rc->min_length[H264_TYPE_BVOP-1] > 8)
		rc->min_length[H264_TYPE_BVOP-1] = 8;
...
scaled_curve_apply_advanced_parameters(rc_2pass2_t * rc)
{
	...
	for (i=0; i<rc->num_frames; i++) {
		...
		if (s->type == H264_TYPE_IVOP) {
			...
			if (ivop_boost + s->scaled_length < rc->min_length[H264_TYPE_IVOP-1])
				ivop_boost = rc->min_length[H264_TYPE_IVOP-1] - s->scaled_length;
	 ...
	 * movie_curve) */
	rc->pb_iboost_tax_ratio  = (double)rc->tot_scaled_length[H264_TYPE_PVOP-1];
	rc->pb_iboost_tax_ratio += (double)rc->tot_scaled_length[H264_TYPE_BVOP-1];
	...
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- IFrame boost tax ratio:%.2f\n",
			rc->pb_iboost_tax_ratio);
	...
	DPRINTF(H264_DEBUG_RC, "[H264 rc] -- Assymetric tax ratio:%.2f\n", rc->assymetric_tax_ratio);
...
check_curve_for_vbv_compliancy(rc_2pass2_t * rc, const float fps)
{
	...
	DPRINTF(H264_DEBUG_RC, "[H264 rc] Minimum buffer fill: %f bytes\n", vbvmin);
...
scale_curve_for_vbv_compliancy(rc_2pass2_t * rc, const float fps)
{
	...
	int last_scene = -999;
	for (i=0; i<rc->num_frames; i++) {
		if ((rc->stats[i].type == H264_TYPE_IVOP) && (i-last_scene>min_scenelength)) {
			last_scene = i;
			num_scenes++;
		}
	}
...
#define R(k,n) (((n)+1-(k))*r0)     /* how much enters the buffer between frame k and n */
	for (l=0; l<num_scenes;l++) {
		...
		DPRINTF(H264_DEBUG_RC, "[H264 rc] Scene %d (Frames %d-%d): VBVfactor %f\n",
...
#undef R
	...
	do {
		...
		if (S_elig < 1.f) {
			DPRINTF(H264_DEBUG_RC, "[H264 rc] Everything underflowing.\n");
			free(scenefactor);
			free(scenestart);
			free(scenelength);
			return(-2);
		...
		DPRINTF(H264_DEBUG_RC, "[H264 rc] Moving %.0f kB to avoid buffer underflow, correction factor: %.5f\n",
			   	S_red/1024.f, f_red);
...
stats_print(rc_2pass2_t * rc)
{
	...
	for (i=0; i<rc->num_frames; i++) {
		twopass_stat_t *s = &rc->stats[i];
		DPRINTF(H264_DEBUG_RC, "[H264 rc] -- frame:%d type:%c quant:%d stats:%d scaled:%d desired:%d actual:%d overflow(%c):%.2f\n",
