 ...
 *  Dirk Knop <dknop@gwdg.de> for the H264 vfw dynamic library.
 *
...
static int rc_2pass1_create(H264_plg_create_t * create, rc_2pass1_t ** handle)
{
    H264_plugin_2pass1_t * param = (H264_plugin_2pass1_t *)create->param;
	rc_2pass1_t * rc;
	...
	/* I swear H264core isn't buggy, but when using mencoder+H2644 i observe
	 * this weird bug.
	 *
	 ...
	 *    + H2644 module opens H264-twopass.stats and writes stats in it.
	 *  - mencoder detects the second sequence and initialize a second
	 *    module and stops the old encoder
	 *    + new H2644 module opens a new H264-twopass.stats, old H2644
	 *      module closes it
	 *
	 ...
	 * The File Header
	 */
	fprintf(rc->stat_file, "# H264 2pass stat file (core version %d.%d.%d)\n",
			H264_VERSION_MAJOR(H264_VERSION),
			H264_VERSION_MINOR(H264_VERSION),
			H264_VERSION_PATCH(H264_VERSION));
...
static int rc_2pass1_destroy(rc_2pass1_t * rc, H264_plg_destroy_t * destroy)
{
	if (rc->stat_file) {
		if (fclose(rc->stat_file) == EOF) {
			DPRINTF(H264_DEBUG_RC, "Error closing stats file (%s)", strerror(errno));
		}
	}
...
static int rc_2pass1_before(rc_2pass1_t * rc, H264_plg_data_t * data)
{
	 if (data->quant <= 0) {
		if (data->zone && data->zone->mode == H264_ZONE_QUANT) {
			...
			data->motion_flags &= ~H264_ME_CHROMA_PVOP;
			data->motion_flags &= ~H264_ME_CHROMA_BVOP;
			data->motion_flags &= ~H264_ME_USESQUARES16;
			data->motion_flags &= ~H264_ME_ADVANCEDDIAMOND16;
			data->motion_flags &= ~H264_ME_EXTSEARCH16;

			/* And enable fast replacements */
			data->motion_flags |= H264_ME_FAST_MODEINTERPOLATE;
			data->motion_flags |= H264_ME_SKIP_DELTASEARCH;
			data->motion_flags |= H264_ME_FASTREFINE16;
			data->motion_flags |= H264_ME_BFRAME_EARLYSTOP;

			/* Now VOP flags (no fast replacements) */
			data->vop_flags &= ~H264_VOP_MODEDECISION_RD;
			data->vop_flags &= ~H264_VOP_RD_BVOP;
			data->vop_flags &= ~H264_VOP_FAST_MODEDECISION_RD;
			data->vop_flags &= ~H264_VOP_TRELLISQUANT;
			data->vop_flags &= ~H264_VOP_INTER4V;
			data->vop_flags &= ~H264_VOP_HQACPRED;
			...
			data->vol_flags &= ~H264_VOL_QUARTERPEL;
#endif
			data->vol_flags &= ~H264_VOL_GMC;
#endif
		}
	}
	 return(0);
...
static int rc_2pass1_after(rc_2pass1_t * rc, H264_plg_data_t * data)
{
	char type;
	H264_enc_stats_t *stats = &data->stats;
...
int H264_plugin_2pass1(void * handle, int opt, void * param1, void * param2)
{
    switch(opt)
    {
    case H264_PLG_INFO :
    ...
    case H264_PLG_CREATE :
        return rc_2pass1_create((H264_plg_create_t*)param1, param2);

    case H264_PLG_DESTROY :
        return rc_2pass1_destroy((rc_2pass1_t*)handle, (H264_plg_destroy_t*)param1);

    case H264_PLG_BEFORE :
        return rc_2pass1_before((rc_2pass1_t*)handle, (H264_plg_data_t*)param1);

    case H264_PLG_AFTER :
        return rc_2pass1_after((rc_2pass1_t*)handle, (H264_plg_data_t*)param1);
    }
