...
static int rc_fixed_create(H264_plg_create_t * create, rc_fixed_t ** handle)
{
    H264_plugin_fixed_t * param = (H264_plugin_fixed_t *)create->param;
	rc_fixed_t * rc;
	int quant_low;
...
static int rc_fixed_destroy(rc_fixed_t * rc, H264_plg_destroy_t * destroy)
{
	free(rc);
	return(0);
}
...
static int rc_fixed_before(rc_fixed_t * rc, H264_plg_data_t * data)
{
    data->quant = rc->quant[data->frame_num%rc->nquant];
    data->type = H264_TYPE_AUTO;
    return 0;
}


int H264_plugin_fixed(void * handle, int opt, void * param1, void * param2)
{
    switch(opt)
    {
    case H264_PLG_INFO :
    ...
    case H264_PLG_CREATE :
        return rc_fixed_create((H264_plg_create_t*)param1, param2);

    case H264_PLG_DESTROY :
        return rc_fixed_destroy((rc_fixed_t*)handle, (H264_plg_destroy_t*)param1);

    case H264_PLG_BEFORE :
        return rc_fixed_before((rc_fixed_t*)handle, (H264_plg_data_t*)param1);
