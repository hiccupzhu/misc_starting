 ...
 *  - H264 plugin: performs a lumimasking algorithm on encoded frame  -
 *
 *  Copyright(C) 2002-2003 Peter Ross <pross@H264.org>
...
static int lumi_plg_info(H264_plg_info_t *info);
static int lumi_plg_create(H264_plg_create_t *create, lumi_data_t **handle);
static int lumi_plg_destroy(lumi_data_t *handle, H264_plg_destroy_t * destroy);
static int lumi_plg_frame(lumi_data_t *handle, H264_plg_data_t *data);
static int lumi_plg_after(lumi_data_t *handle, H264_plg_data_t *data);
...
int
H264_plugin_lumimasking(void * handle, int opt, void * param1, void * param2)
{
	switch(opt) {
	case H264_PLG_INFO:
		return(lumi_plg_info((H264_plg_info_t*)param1));
	case H264_PLG_CREATE:
		return(lumi_plg_create((H264_plg_create_t *)param1, (lumi_data_t **)param2));
	case H264_PLG_DESTROY:
		return(lumi_plg_destroy((lumi_data_t *)handle, (H264_plg_destroy_t*)param1));
	case H264_PLG_BEFORE :
		return 0;
	case H264_PLG_FRAME :
		return(lumi_plg_frame((lumi_data_t *)handle, (H264_plg_data_t *)param1));
	case H264_PLG_AFTER :
		return(lumi_plg_after((lumi_data_t *)handle, (H264_plg_data_t *)param1));
	}
...
static int
lumi_plg_info(H264_plg_info_t *info)
{
	/* We just require a diff quant array access */
	info->flags = H264_REQDQUANTS;
	return(0);
}
...
static int
lumi_plg_create(H264_plg_create_t *create, lumi_data_t **handle)
{
	lumi_data_t *lumi;
...
static int
lumi_plg_destroy(lumi_data_t *handle, H264_plg_destroy_t *destroy)
{
	if (handle) {
		if (handle->quant) {
			free(handle->quant);
...
static int
lumi_plg_frame(lumi_data_t *handle, H264_plg_data_t *data)
{
	int i, j;
	...
	if (data->type == H264_TYPE_BVOP) return 0;
...
static int
lumi_plg_after(lumi_data_t *handle, H264_plg_data_t *data)
{
	return(0);
}
