 ...
 *  - Encoder main module -
 *
 *  Copyright(C) 2002	  Michael Militzer <isibaar@H264.org>
 *			   2002-2003 Peter Ross <pross@H264.org>
 ...
 * H264 encoder paremeters according to the H264_ENC_PARAM input parameter.
 *
 ...
 * Returned values :
 *	- 0				- no errors
 *	- H264_ERR_MEMORY - the libc could not allocate memory, the function
...
int
enc_create(H264_enc_create_t * create)
{
	Encoder *pEnc;
  int n;

	if (H264_VERSION_MAJOR(create->version) != 1) /* v1.x.x */
		return H264_ERR_VERSION;
	...
	pEnc = (Encoder *) H264_malloc(sizeof(Encoder), CACHE_LINE);
	if (pEnc == NULL)
		return H264_ERR_MEMORY;
  ...
  if ((pEnc->mbParam.global_flags & H264_GLOBAL_PACKED))
    pEnc->mbParam.global_flags |= H264_GLOBAL_DIVX5_USERDATA;
	...
	if(create->num_zones > 0) {
		...
		pEnc->zones = H264_malloc(sizeof(H264_enc_zone_t) * pEnc->num_zones, CACHE_LINE);
		if (pEnc->zones == NULL)
			goto H264_err_memory0;
		memcpy(pEnc->zones, create->zones, sizeof(H264_enc_zone_t) * pEnc->num_zones);
	} else {
		pEnc->num_zones = 0;
		pEnc->zones = NULL;
	}
	...
	if(create->num_plugins > 0) {
		...
		pEnc->plugins = H264_malloc(sizeof(H264_enc_plugin_t) * pEnc->num_plugins, CACHE_LINE);
	...
	for (n=0; n<pEnc->num_plugins;n++) {
		H264_plg_create_t pcreate;
		H264_plg_info_t pinfo;

		memset(&pinfo, 0, sizeof(H264_plg_info_t));
		pinfo.version = H264_VERSION;
		if (create->plugins[n].func(NULL, H264_PLG_INFO, &pinfo, NULL) >= 0) {
			pEnc->mbParam.plugin_flags |= pinfo.flags;
		}

		memset(&pcreate, 0, sizeof(H264_plg_create_t));
		pcreate.version = H264_VERSION;
		...
		if (create->plugins[n].func(NULL, H264_PLG_CREATE, &pcreate, &pEnc->plugins[n].param) >= 0) {
			pEnc->plugins[n].func = create->plugins[n].func;
		}
	}

	if ((pEnc->mbParam.global_flags & H264_GLOBAL_EXTRASTATS_ENABLE) ||
		(pEnc->mbParam.plugin_flags & H264_REQPSNR)) {
		pEnc->mbParam.plugin_flags |= H264_REQORIGINAL; /* psnr calculation requires the original */
	}

	/* temp dquants */
	if ((pEnc->mbParam.plugin_flags & H264_REQDQUANTS)) {
		pEnc->temp_dquants = (int *) H264_malloc(pEnc->mbParam.mb_width *
						pEnc->mbParam.mb_height * sizeof(int), CACHE_LINE);
		if (pEnc->temp_dquants==NULL)
			goto H264_err_memory1a;
	}
	...
	pEnc->current = H264_malloc(sizeof(FRAMEINFO), CACHE_LINE);
	pEnc->reference = H264_malloc(sizeof(FRAMEINFO), CACHE_LINE);

	if (pEnc->current == NULL || pEnc->reference == NULL)
		goto H264_err_memory1;

	/* allocate macroblock memory */

	pEnc->current->mbs =
		H264_malloc(sizeof(MACROBLOCK) * pEnc->mbParam.mb_width *
...
/* Create full bitplane for GMC, this might be wasteful */
	...
	if (pEnc->mbParam.max_bframes > 0) {
		...
		pEnc->bframes =
			H264_malloc(pEnc->mbParam.max_bframes * sizeof(FRAMEINFO *),
						CACHE_LINE);
		...
		for (n = 0; n < pEnc->mbParam.max_bframes; n++) {
			pEnc->bframes[n] = H264_malloc(sizeof(FRAMEINFO), CACHE_LINE);

			if (pEnc->bframes[n] == NULL)
				goto H264_err_memory4;
	...
	pEnc->queue =
		H264_malloc((pEnc->mbParam.max_bframes+1) * sizeof(QUEUEINFO),
					CACHE_LINE);
	...
	/*
	 * We handle all H264_ERR_MEMORY here, this makes the code lighter
	 */
	...
	if (pEnc->mbParam.max_bframes > 0) {
		...
		for (i = 0; i < pEnc->mbParam.max_bframes; i++) {
			...
			image_destroy(&pEnc->bframes[i]->image, pEnc->mbParam.edged_width,
						  pEnc->mbParam.edged_height);
			H264_free(pEnc->bframes[i]->mbs);
			H264_free(pEnc->bframes[i]);
		}

		H264_free(pEnc->bframes);
	}
...
/* destroy GMC image */
  ...
  H264_err_memory2a:
	H264_free(pEnc->mbParam.mpeg_quant_matrices);

  H264_err_memory2:
	H264_free(pEnc->current->mbs);
	H264_free(pEnc->reference->mbs);

  H264_err_memory1:
	H264_free(pEnc->current);
	H264_free(pEnc->reference);

  H264_err_memory1a:
	if ((pEnc->mbParam.plugin_flags & H264_REQDQUANTS)) {
		H264_free(pEnc->temp_dquants);
	}

  H264_err_memory0:
	for (n=0; n<pEnc->num_plugins;n++) {
		if (pEnc->plugins[n].func) {
			pEnc->plugins[n].func(pEnc->plugins[n].param, H264_PLG_DESTROY, NULL, NULL);
		}
	}
	H264_free(pEnc->plugins);

	H264_free(pEnc->zones);
...
enc_destroy(Encoder * pEnc)
{
	...
	if (pEnc->num_plugins>0) {
		H264_plg_destroy_t pdestroy;
		memset(&pdestroy, 0, sizeof(H264_plg_destroy_t));

		pdestroy.version = H264_VERSION;
		pdestroy.num_frames = pEnc->m_framenum;

		for (i=0; i<pEnc->num_plugins;i++) {
			if (pEnc->plugins[i].func) {
				pEnc->plugins[i].func(pEnc->plugins[i].param, H264_PLG_DESTROY, &pdestroy, NULL);
			}
		}
		H264_free(pEnc->plugins);
	}
...
static void call_plugins(Encoder * pEnc, FRAMEINFO * frame, IMAGE * original,
						 int opt, int * type, int * quant, H264_enc_stats_t * stats)
{
	unsigned int i, j;
	H264_plg_data_t data;

	/* set data struct */

	memset(&data, 0, sizeof(H264_plg_data_t));
	data.version = H264_VERSION;
	...
	data.reference.csp = H264_CSP_PLANAR;
	...
	data.current.csp = H264_CSP_PLANAR;
	...
	if (opt == H264_PLG_BEFORE) {
		...
		if ((pEnc->mbParam.plugin_flags & H264_REQORIGINAL)) {
			data.original.csp = H264_CSP_PLANAR;
		...
		if ((frame->vol_flags & H264_VOL_EXTRASTATS) ||
			(pEnc->mbParam.plugin_flags & H264_REQPSNR)) {
 ...
 *	- >0			   - output bytes
 *	- 0				- no output
 *	- H264_ERR_VERSION - wrong version passed to core
 *	- H264_ERR_END	 - End of stream reached before end of coding
 *	- H264_ERR_FORMAT  - the image subsystem reported the image had a wrong
 *						 format
 ****************************************************************************/
...
enc_encode(Encoder * pEnc,
			   H264_enc_frame_t * xFrame,
			   H264_enc_stats_t * stats)
{
	H264_enc_frame_t * frame;
	int type;
	Bitstream bs;

	if (H264_VERSION_MAJOR(xFrame->version) != 1 || (stats && H264_VERSION_MAJOR(stats->version) != 1))	/* v1.x.x */
		return H264_ERR_VERSION;
	...
	if (xFrame->input.csp != H264_CSP_NULL)
	{
		...
		if (image_input
			...
			xFrame->input.csp, xFrame->vol_flags & H264_VOL_INTERLACING))
		{
			emms();
			return H264_ERR_FORMAT;
		}
		stop_conv_timer();

		if ((xFrame->vop_flags & H264_VOP_CHROMAOPT)) {
...
repeat:
	...
	if (pEnc->flush_bframes)
	{
		if (pEnc->bframenum_head < pEnc->bframenum_tail) {
			...
			DPRINTF(H264_DEBUG_DEBUG,"*** BFRAME (flush) bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
			...
			call_plugins(pEnc, pEnc->bframes[pEnc->bframenum_head], &pEnc->sOriginal2, H264_PLG_AFTER, NULL, NULL, stats);
			pEnc->bframenum_head++;
		...
		*/
		...
		if ((pEnc->mbParam.global_flags & H264_GLOBAL_PACKED && pEnc->bframenum_tail > 0)) {
			int tmp;
			int bits;

			DPRINTF(H264_DEBUG_DEBUG,"*** EMPTY bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
				pEnc->bframenum_head, pEnc->bframenum_tail,
				pEnc->queue_head, pEnc->queue_tail, pEnc->queue_size);
			...
			call_plugins(pEnc, pEnc->current, &pEnc->sOriginal, H264_PLG_AFTER, NULL, NULL, stats);
	...
	{
		if (xFrame->input.csp == H264_CSP_NULL)	/* no futher input */
		{

			DPRINTF(H264_DEBUG_DEBUG,"*** FINISH bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
				pEnc->bframenum_head, pEnc->bframenum_tail,
				pEnc->queue_head, pEnc->queue_tail, pEnc->queue_size);

			if (!(pEnc->mbParam.global_flags & H264_GLOBAL_PACKED) && pEnc->mbParam.max_bframes > 0) {
				call_plugins(pEnc, pEnc->current, &pEnc->sOriginal, H264_PLG_AFTER, NULL, NULL, stats);
			}
			...
			if (pEnc->bframenum_tail > 0) {
				...
				DPRINTF(H264_DEBUG_DEBUG,"*** PFRAME bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
				pEnc->bframenum_head, pEnc->bframenum_tail,
				pEnc->queue_head, pEnc->queue_tail, pEnc->queue_size);
			...
			DPRINTF(H264_DEBUG_DEBUG, "*** END\n");

			emms();
			return H264_ERR_END;	/* end of stream reached */
		}
	...
	call_plugins(pEnc, pEnc->current, NULL, H264_PLG_BEFORE, &type, (int*)&pEnc->current->quant, stats);

	if (type > 0){ 	/* H264_TYPE_?VOP */
		type = type2coding(type);	/* convert H264_TYPE_?VOP to bitstream coding type */
	} else{		/* H264_TYPE_AUTO */
	...
	if ((pEnc->current->vop_flags & H264_VOP_DEBUG)) {
	...
	if (type == B_VOP) {
		...
		DPRINTF(H264_DEBUG_DEBUG,"*** BFRAME (store) bf: head=%i tail=%i   queue: head=%i tail=%i size=%i  quant=%i\n",
				pEnc->bframenum_head, pEnc->bframenum_tail,
		...
		DPRINTF(H264_DEBUG_DEBUG,"*** XXXXXX bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
				pEnc->bframenum_head, pEnc->bframenum_tail,
				pEnc->queue_head, pEnc->queue_tail, pEnc->queue_size);
	...
	if (!(pEnc->mbParam.global_flags & H264_GLOBAL_PACKED) && pEnc->mbParam.max_bframes > 0)
	{
		if (pEnc->current->stamp > 0) {
			call_plugins(pEnc, pEnc->reference, &pEnc->sOriginal, H264_PLG_AFTER, NULL, NULL, stats);
		}
		else
			stats->type = H264_TYPE_NOTHING;
	}
	...
	if (type == I_VOP && (pEnc->mbParam.global_flags & H264_GLOBAL_CLOSED_GOP) && pEnc->bframenum_tail > 0) {
	...
	if (type == I_VOP) {
		...
		DPRINTF(H264_DEBUG_DEBUG,"*** IFRAME bf: head=%i tail=%i   queue: head=%i tail=%i size=%i\n",
				pEnc->bframenum_head, pEnc->bframenum_tail,
				pEnc->queue_head, pEnc->queue_tail, pEnc->queue_size);
		...
		/* Aspect ratio */
		switch(frame->par) {
		case H264_PAR_11_VGA:
		case H264_PAR_43_PAL:
		case H264_PAR_43_NTSC:
		case H264_PAR_169_PAL:
		case H264_PAR_169_NTSC:
		case H264_PAR_EXT:
			pEnc->mbParam.par = frame->par;
			break;
		default:
			pEnc->mbParam.par = H264_PAR_11_VGA;
			break;
		}

		/* For extended PAR only, we try to sanityse/simplify par values */
		if (pEnc->mbParam.par == H264_PAR_EXT) {
		...
		if ((pEnc->mbParam.vol_flags & H264_VOL_MPEGQUANT)) {
		...
		if (!(pEnc->current->vol_flags & H264_VOL_INTERLACING))
			pEnc->current->vop_flags &= ~(H264_VOP_TOPFIELDFIRST|H264_VOP_ALTERNATESCAN);
		...
		FrameCodeI(pEnc, &bs);
		xFrame->out_flags |= H264_KEYFRAME;
...
static void SetMacroblockQuants(MBParam * const pParam, FRAMEINFO * frame)
{
	...
	int quant = frame->mbs[0].quant; /* set by H264_PLG_FRAME */
	if (quant > 31)
		   ...
		   Bitstream * bs)
{
	...
	call_plugins(pEnc, pEnc->current, NULL, H264_PLG_FRAME, NULL, NULL, NULL);
		   ...
		   Bitstream * bs)
{
	...
	if ((current->vop_flags & H264_VOP_HALFPEL)) {
		if (reference->is_interpolated != current->rounding_type) {
			...
			image_interpolate(pRef, &pEnc->vInterH, &pEnc->vInterV,
							  ...
							  pParam->edged_height,
							  (pParam->vol_flags & H264_VOL_QUARTERPEL),
	...
	start_timer();
	if (current->vol_flags & H264_VOL_GMC )	/* GMC only for S(GMC)-VOPs */
	{	int gmcval;
		current->warp = GlobalMotionEst( current->mbs, pParam, current, reference,
								 &pEnc->vInterH, &pEnc->vInterV, &pEnc->vInterHV);

		if (current->motion_flags & H264_ME_GME_REFINE) {
			 ...
			 (current->warp.duv[2].x != 0) || (current->warp.duv[2].y != 0) ) )
		{
			...
			generate_GMCimage(&current->new_gmc_data, &reference->image,
				...
				pParam->m_fcode, ((pParam->vol_flags & H264_VOL_QUARTERPEL)?1:0), 0,
	...
	for (y = 0; y < mb_height; y++) {
		for (x = 0; x < mb_width; x++) {
			...
			MBMotionCompensation(pMB, x, y, &reference->image,
								 ...
								 pParam->height,
								 pParam->edged_width,
								 (current->vol_flags & H264_VOL_QUARTERPEL),
								 current->rounding_type);
			...
			if (current->coding_type == S_VOP)
				...
				const VECTOR * const mv = (pParam->vol_flags & H264_VOL_QUARTERPEL) ?
										pMB->qmvs : pMB->mvs;
				...
				if (current->coding_type == P_VOP) {	/* special rule for P-VOP's SKIP */
					...
					for (k = pEnc->bframenum_head; k < pEnc->bframenum_tail; k++) {
						...
						if (iSAD >= BFRAME_SKIP_THRESHHOLD * pMB->quant) {
							...
							if (pParam->vol_flags & H264_VOL_QUARTERPEL) {
	...
	/* frame drop code */
#if 0
	DPRINTF(H264_DEBUG_DEBUG, "kmu %i %i %i\n", current->sStat.kblks, current->sStat.mblks, current->sStat.ublks);
#endif
	if (current->sStat.kblks + current->sStat.mblks <=
		(pParam->frame_drop_ratio * mb_width * mb_height) / 100 &&
		( (pEnc->bframenum_head >= pEnc->bframenum_tail) || !(pEnc->mbParam.global_flags & H264_GLOBAL_CLOSED_GOP)) )
	{
...
#define BFRAME_DEBUG  	if (!first && fp){ \
	...
	if (pEnc->reference->is_interpolated != 0) {
		...
		image_interpolate(f_ref, &pEnc->f_refh, &pEnc->f_refv, &pEnc->f_refhv,
						  ...
						  (pEnc->mbParam.vol_flags & H264_VOL_QUARTERPEL), 0);
		stop_inter_timer();
	...
	for (y = 0; y < pEnc->mbParam.mb_height; y++) {
		for (x = 0; x < pEnc->mbParam.mb_width; x++) {
			...
			if (mb->cbp != 0 || pEnc->mbParam.plugin_flags & H264_REQORIGINAL) {
			...
			if (mb->mode == MODE_DIRECT && (mb->cbp | mb->pmvs[3].x | mb->pmvs[3].y) == 0)
				...
				if (frame->vop_flags & H264_VOP_GREYSCALE)
