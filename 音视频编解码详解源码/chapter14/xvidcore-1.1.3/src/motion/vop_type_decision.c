 ...
 *
 *  Copyright(C) 2002-2003 Radoslaw Czyz <H264@syskin.cjb.net>
 *
				...
				SearchData * const Data)
{
	...
	if (*Data->iMinSAD > 200) {
		...
		if (*Data->iMinSAD > 500) { /* diamond only if needed */
			...
			H264_me_DiamondSearch(Data->currentMV->x, Data->currentMV->y, Data, mask, CheckCandidate32I);
		} else simplicity++;

		if (*Data->iMinSAD > 500) /* refinement from 2-pixel to 1-pixel */
			H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate32I, 0);
		else simplicity++;
	} else simplicity++;
			...
			const MACROBLOCK * const prev_mbs)
{
	...
	Data.qpel = (pParam->vol_flags & H264_VOL_QUARTERPEL)? 1: 0;
