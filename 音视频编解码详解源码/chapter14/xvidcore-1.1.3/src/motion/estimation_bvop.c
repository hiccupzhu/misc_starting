...
CheckCandidateInt(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	if (!data->qpel_precision) {
		...
		ReferenceF = H264_me_interpolate16x16qpel(xf, yf, 0, data);
		current = data->currentQMV + Direction - 1;
		ReferenceB = H264_me_interpolate16x16qpel(xb, yb, 1, data);
		xcf = xf/2; ycf = yf/2;
		xcb = xb/2; ycb = yb/2;
	}
...
CheckCandidateDirect(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	for (k = 0; k < 4; k++) {
		...
		if (data->qpel) {
			...
			if (data->qpel_precision) {
				ReferenceF = H264_me_interpolate8x8qpel(mvs.x, mvs.y, k, 0, data);
				ReferenceB = H264_me_interpolate8x8qpel(b_mvs.x, b_mvs.y, k, 1, data);
				goto done;
			}
...
CheckCandidateDirectno4v(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	if (data->qpel) {
		...
		if (data->qpel_precision) {
			ReferenceF = H264_me_interpolate16x16qpel(mvs.x, mvs.y, 0, data);
			ReferenceB = H264_me_interpolate16x16qpel(b_mvs.x, b_mvs.y, 1, data);
			goto done;
		}
		mvs.x >>=1; mvs.y >>=1; b_mvs.x >>=1; b_mvs.y >>=1; // qpel->hpel
	} else {
					  ...
					  const MACROBLOCK * const b_mb)
{
	...
	lambda = H264_me_lambda_vec16[b_mb->quant];
			...
			SearchData * const Data)
{
	if(!Data->qpel) {
		...
		if (MotionFlags & H264_ME_HALFPELREFINE16)
				H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate16no4v, 0);
	} else {
		/* qpel mode */
		if(MotionFlags & H264_ME_FASTREFINE16) {
			...
			if(MotionFlags & H264_ME_QUARTERPELREFINE16) {
				/* full */
				if (MotionFlags & H264_ME_HALFPELREFINE16) {
					H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate16no4v, 0); /* hpel part */
					Data->currentQMV->x = 2*Data->currentMV->x;
					Data->currentQMV->y = 2*Data->currentMV->y;
				}
				get_range(&Data->min_dx, &Data->max_dx, &Data->min_dy, &Data->max_dy, x, y, 4,
				...
				H264_me_SubpelRefine(Data->currentQMV[0], Data, CheckCandidate16no4v, 0); /* qpel part */
			}
		}
	}
	if (Data->iMinSAD[0] < *best_sad) *best_sad = Data->iMinSAD[0];
						...
						SearchData * const Data)
{
	...
	if (Data->qpel) {
		...
		if (MotionFlags & H264_ME_QUARTERPELREFINE16) {
			H264_me_SubpelRefine(Data->currentQMV[0], Data, CheckCandidateInt, 1);

			get_range(&Data->min_dx, &Data->max_dx, &Data->min_dy, &Data->max_dy, 
				x, y, 4, pParam->width, pParam->height, Data->bFcode, 2);

			H264_me_SubpelRefine(Data->currentQMV[1], Data, CheckCandidateInt, 2);
		}
	}
					 ...
					 const IMAGE * const b_refHV)
{
	...
	Data_d.qpel = pParam->vol_flags & H264_VOL_QUARTERPEL ? 1 : 0;
	Data_d.rounding = 0;
	Data_d.chroma = frame->motion_flags & H264_ME_CHROMA_BVOP;
	Data_d.iQuant = frame->quant;
	Data_d.quant_sq = frame->quant*frame->quant;
	Data_d.dctSpace = dct_space;
	Data_d.quant_type = !(pParam->vol_flags & H264_VOL_MPEGQUANT);
...
/* direct search comes first, because it (1) checks for SKIP-mode
	and (2) sets very good predictions for forward and backward search */
			...
			if (frame->motion_flags&H264_ME_BFRAME_EARLYSTOP)
				fb_thresh = best_sad;
			else
			...
			if (((Data_i.iMinSAD[0] < best_sad +(best_sad>>3)) && !(frame->motion_flags&H264_ME_FAST_MODEINTERPOLATE))
			...
			if (Data_d.iMinSAD[0] <= 2*best_sad)
				if ((!(frame->motion_flags&H264_ME_SKIP_DELTASEARCH) && (best_sad > 750))
					|| (best_sad > 1000))
			...
			if (frame->vop_flags & H264_VOP_RD_BVOP) 
