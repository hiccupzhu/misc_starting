 ...
 *
 *  Copyright(C) 2003 Radoslaw Czyz <H264@syskin.cjb.net>
 *               2003 Michael Militzer <michael@H264.org>
 *
				...
				const unsigned int quant_sq)
{
	...
	if (quant_type) sum = quant_h264_inter(coeff, data, quant, mpeg_quant_matrices);
	...
	if (sum > 0) {
		*cbp |= 1 << (5 - block);
		...
		if (quant_type) dequant_h264_inter(dqcoeff, coeff, quant, mpeg_quant_matrices);
					...
					const unsigned int quant_sq)
{
	...
	if (quant_type) {
		quant_h264_intra(qcoeff, coeff, quant, iDcScaler, mpeg_quant_matrices);
		dequant_h264_intra(dqcoeff, qcoeff, quant, iDcScaler, mpeg_quant_matrices);
	} else {
...
CheckCandidateRD16(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	if (!data->qpel_precision) {
		...
		ptr = H264_me_interpolate16x16qpel(x, y, 0, data);
	...
	rd += BITS_MULT * (H264_cbpy_tab[15-(cbp>>2)].len - 2);
...
CheckCandidateRD8(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	if (!data->qpel_precision) {
		...
		ptr = H264_me_interpolate8x8qpel(x, y, 0, 0, data);
		current = data->currentQMV;
	}
			...
			const uint32_t MotionFlags)
{
	...
	if (Data->qpel) {
		...
		if (MotionFlags & (H264_ME_HALFPELREFINE16_RD | H264_ME_EXTSEARCH_RD)) { /* we have to prepare for halfpixel-precision search */
	...
	if (MotionFlags&H264_ME_EXTSEARCH_RD)
		H264_me_SquareSearch(Data->currentMV->x, Data->currentMV->y, Data, 255, CheckCandidateRD16);

	if (MotionFlags&H264_ME_HALFPELREFINE16_RD)
		H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidateRD16, 0);

	if (Data->qpel) {
		if (MotionFlags&(H264_ME_EXTSEARCH_RD | H264_ME_HALFPELREFINE16_RD)) { /* there was halfpel-precision search */
		...
		if (MotionFlags & H264_ME_QUARTERPELREFINE16_RD) {
			if (MotionFlags & H264_ME_FASTREFINE16)
				FullRefine_Fast(Data, CheckCandidateRD16, 0);
			else 
				H264_me_SubpelRefine(Data->currentQMV[0], Data, CheckCandidateRD16, 0);
		}
	}

	if (MotionFlags&H264_ME_CHECKPREDICTION_RD) { /* let's check vector equal to prediction */
				...
				const VECTOR * const backup)
{
	...
	for (i = 0; i < 4; i++) { /* for all luma blocks */
		...
		*Data8->iMinSAD += BITS_MULT * t;
		...
		if (Data8->qpel) {
			...
			if (MotionFlags&H264_ME_HALFPELREFINE8_RD || (MotionFlags&H264_ME_EXTSEARCH8 && MotionFlags&H264_ME_EXTSEARCH_RD)) { /* halfpixel motion search follows */
				...
				if (MotionFlags & H264_ME_EXTSEARCH8 && MotionFlags & H264_ME_EXTSEARCH_RD)
					H264_me_SquareSearch(Data8->currentMV->x, Data8->currentMV->x, Data8, 255, CheckCandidateRD8);

				if (MotionFlags & H264_ME_HALFPELREFINE8_RD)
					H264_me_SubpelRefine(Data->currentMV[0], Data8, CheckCandidateRD8, 0);
			...
			if (MotionFlags & H264_ME_QUARTERPELREFINE8_RD) {
				if (MotionFlags & H264_ME_FASTREFINE8)
					FullRefine_Fast(Data8, CheckCandidateRD8, 0);
				else H264_me_SubpelRefine(Data->currentQMV[0], Data8, CheckCandidateRD8, 0);
			}
			...
			if (MotionFlags & H264_ME_EXTSEARCH8 && MotionFlags & H264_ME_EXTSEARCH_RD) /* extsearch */
				H264_me_SquareSearch(Data8->currentMV->x, Data8->currentMV->x, Data8, 255, CheckCandidateRD8);

			if (MotionFlags & H264_ME_HALFPELREFINE8_RD)
				H264_me_SubpelRefine(Data->currentMV[0], Data8, CheckCandidateRD8, 0); /* halfpel refinement */
		}

		/* checking vector equal to predicion */
		if (i != 0 && MotionFlags & H264_ME_CHECKPREDICTION_RD) {
	...
	bits += BITS_MULT * (H264_cbpy_tab[15-(cbp>>2)].len - 2); /* 2 were added before */
			 ...
			 const int x, const int y, const int mb_width)
{
	...
	bits1 += BITS_MULT * (H264_cbpy_tab[cbp[0]>>2].len - 2); /* two bits were added before */
	bits2 += BITS_MULT * (H264_cbpy_tab[cbp[1]>>2].len - 2);
...
findRD_gmc(SearchData * const Data, const IMAGE * const vGMC, const int x, const int y)
{
	...
	bits += BITS_MULT * (H264_cbpy_tab[15-(cbp>>2)].len - 2);
...
void
H264_me_ModeDecision_RD(SearchData * const Data,
				...
				const int coding_type)
{
	...
	Data->scan_table = VopFlags & H264_VOP_ALTERNATESCAN ?
...
void
H264_me_ModeDecision_Fast(SearchData * const Data,
			MACROBLOCK * const pMB,
			const MACROBLOCK * const pMBs,
			const int x, const int y,
			const MBParam * const pParam,
