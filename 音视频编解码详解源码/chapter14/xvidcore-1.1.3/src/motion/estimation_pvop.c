 ...
 *               2002 Michael Militzer <michael@H264.org>
 *               2002-2003 Radoslaw Czyz <H264@syskin.cjb.net>
 *
...
static const int H264_me_lambda_vec8[32] =
...
CheckCandidate16(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	if (data->qpel_precision) { /* x and y are in 1/4 precision */
		Reference = H264_me_interpolate16x16qpel(x, y, 0, data);
	...
	if (data->chroma) {
		...
		sad += H264_me_ChromaSAD((xc >> 1) + roundtab_79[xc & 0x3],
...
CheckCandidate8(const int x, const int y, SearchData * const data, const unsigned int Direction)
{
	...
	if (!data->qpel_precision) {
		...
		Reference = H264_me_interpolate8x8qpel(x, y, 0, 0, data);
		current = data->currentQMV;
	}
...
int
H264_me_SkipDecisionP(const IMAGE * current, const IMAGE * reference,
				...
				const int skip_sad)
{
	int mode = MODE_INTER;
	int mcsel = 0;
	int inter4v = (VopFlags & H264_VOP_INTER4V) && (pMB->dquant == 0);
	...
	if (skip_possible && (skip_sad < (int)iQuant * MAX_SAD00_FOR_SKIP))
		if ( (100*skip_sad)/(pMB->sad16+1) > FINAL_SKIP_THRESH)
			if (Data->chroma || H264_me_SkipDecisionP(pCurrent, pRef, x, y, Data->iEdgedWidth/2, iQuant)) {
				mode = MODE_NOT_CODED;
				sad = 0;
			}
		...
		SearchData * const Data)
{
	...
	*(Data->iMinSAD) += (Data->lambda8 * i);
	...
	if (MotionFlags & (H264_ME_EXTSEARCH8|H264_ME_HALFPELREFINE8|H264_ME_QUARTERPELREFINE8)) {
		...
		if (MotionFlags & H264_ME_EXTSEARCH8 && (!(MotionFlags & H264_ME_EXTSEARCH_RD))) {

			MainSearchFunc *MainSearchPtr;
			if (MotionFlags & H264_ME_USESQUARES8) MainSearchPtr = H264_me_SquareSearch;
				else if (MotionFlags & H264_ME_ADVANCEDDIAMOND8) MainSearchPtr = H264_me_AdvDiamondSearch;
					else MainSearchPtr = H264_me_DiamondSearch;
		...
		if(!Data->qpel) {
			/* halfpel mode */
			if (MotionFlags & H264_ME_HALFPELREFINE8)
				/* perform halfpel refine of current best vector */
				H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate8, 0);
		} else {
			/* qpel mode */
			Data->currentQMV->x = 2*Data->currentMV->x;
			Data->currentQMV->y = 2*Data->currentMV->y;
			
			if(MotionFlags & H264_ME_FASTREFINE8) {
				/* fast */
				get_range(&Data->min_dx, &Data->max_dx, &Data->min_dy, &Data->max_dy, x, y, 3,
					pParam->width, pParam->height, Data->iFcode, 2);
				FullRefine_Fast(Data, CheckCandidate8, 0);
			} else if(MotionFlags & H264_ME_QUARTERPELREFINE8) {
				/* full */
				if (MotionFlags & H264_ME_HALFPELREFINE8) {
					H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate8, 0); /* hpel part */
					Data->currentQMV->x = 2*Data->currentMV->x;
					Data->currentQMV->y = 2*Data->currentMV->y;
				}
				...
				H264_me_SubpelRefine(Data->currentQMV[0], Data, CheckCandidate8, 0); /* qpel part */
			}
		}
		...
		MACROBLOCK * const pMB)
{
	...
	Data->lambda16 = H264_me_lambda_vec16[pMB->quant];
	Data->lambda8 = H264_me_lambda_vec8[pMB->quant];
	Data->qpel_precision = 0;
	Data->dir = 0;
	...
	if ((!(VopFlags & H264_VOP_MODEDECISION_RD)) && (x | y)) {
...
/* main loop. checking all predictions (but first, which is 0,0 and has been checked in MotionEstimation())*/
	...
	if ((Data->iMinSAD[0] <= threshA) ||
		...
		if (MotionFlags & H264_ME_USESQUARES16) MainSearchPtr = H264_me_SquareSearch;
		else if (MotionFlags & H264_ME_ADVANCEDDIAMOND16) MainSearchPtr = H264_me_AdvDiamondSearch;
			else MainSearchPtr = H264_me_DiamondSearch;
...
/* extended search, diamond starting in 0,0 and in prediction.
	...
	which makes it more different than the diamond above */
		...
		if (MotionFlags & H264_ME_EXTSEARCH16) {
			int32_t bSAD;
			...
			if (!(MVequal(startMV, backupMV))) {
				...
				H264_me_DiamondSearch(startMV.x, startMV.y, Data, 255, CheckCandidate);
	...
	if(!Data->qpel) {
		/* halfpel mode */
		if (MotionFlags & H264_ME_HALFPELREFINE16)
				H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate, 0);	
	} else {
		/* qpel mode */
		...
		if(MotionFlags & H264_ME_FASTREFINE16 && MotionFlags & H264_ME_QUARTERPELREFINE16) {
			/* fast */
			get_range(&Data->min_dx, &Data->max_dx, &Data->min_dy, &Data->max_dy, x, y, 4,
						pParam->width, pParam->height, Data->iFcode, 2);
			FullRefine_Fast(Data, CheckCandidate, 0);
		} else {
			if(MotionFlags & (H264_ME_QUARTERPELREFINE16 | H264_ME_QUARTERPELREFINE16_RD)) {
				/* full */
				if (MotionFlags & H264_ME_HALFPELREFINE16) {
					H264_me_SubpelRefine(Data->currentMV[0], Data, CheckCandidate, 0); /* hpel part */
					for(i = 0; i < 5; i++) {
				...
				get_range(&Data->min_dx, &Data->max_dx, &Data->min_dy, &Data->max_dy, x, y, 4,
							pParam->width, pParam->height, Data->iFcode, 2);
				Data->qpel_precision = 1;
				if(MotionFlags & H264_ME_QUARTERPELREFINE16)
					H264_me_SubpelRefine(Data->currentQMV[0], Data, CheckCandidate, 0); /* qpel part */
			}
		}
	}

	if (Data->iMinSAD[0] < (int32_t)pMB->quant * 30 * ((MotionFlags & H264_ME_FASTREFINE16) ? 8 : 1))
		inter4v = 0;
	...
	if (inter4v) {
		...
		if ((Data->chroma) && (!(VopFlags & H264_VOP_MODEDECISION_RD))) {
			...
			Data->iMinSAD[1] += H264_me_ChromaSAD((sumx >> 3) + roundtab_76[sumx & 0xf],
...
MakeGoodMotionFlags(const uint32_t MotionFlags, const uint32_t VopFlags, const uint32_t VolFlags)
{
	...
	if (!(VopFlags & H264_VOP_MODEDECISION_RD))
		Flags &= ~(H264_ME_QUARTERPELREFINE16_RD+H264_ME_QUARTERPELREFINE8_RD+H264_ME_HALFPELREFINE16_RD+H264_ME_HALFPELREFINE8_RD+H264_ME_EXTSEARCH_RD);

	if (Flags & H264_ME_EXTSEARCH_RD)
		Flags |= H264_ME_HALFPELREFINE16_RD;

	if (Flags & H264_ME_EXTSEARCH_RD && MotionFlags & H264_ME_EXTSEARCH8)
		Flags |= H264_ME_HALFPELREFINE8_RD;

	if (Flags & H264_ME_HALFPELREFINE16_RD)
		Flags |= H264_ME_QUARTERPELREFINE16_RD;

	if (Flags & H264_ME_HALFPELREFINE8_RD) {
		Flags |= H264_ME_QUARTERPELREFINE8_RD;
		Flags &= ~H264_ME_HALFPELREFINE8;
	}

	if (Flags & H264_ME_QUARTERPELREFINE8_RD)
		Flags &= ~H264_ME_QUARTERPELREFINE8;

	if (Flags & H264_ME_QUARTERPELREFINE16_RD)
		Flags &= ~H264_ME_QUARTERPELREFINE16;

	if (!(VolFlags & H264_VOL_QUARTERPEL))
		Flags &= ~(H264_ME_QUARTERPELREFINE16+H264_ME_QUARTERPELREFINE8+H264_ME_QUARTERPELREFINE16_RD+H264_ME_QUARTERPELREFINE8_RD);

	if (!(VopFlags & H264_VOP_HALFPEL))
		Flags &= ~(H264_ME_EXTSEARCH16+H264_ME_HALFPELREFINE16+H264_ME_HALFPELREFINE8+H264_ME_HALFPELREFINE16_RD+H264_ME_HALFPELREFINE8_RD);

	if (VopFlags & H264_VOP_GREYSCALE)
		Flags &= ~(H264_ME_CHROMA_PVOP + H264_ME_CHROMA_BVOP);

	if (Flags & H264_ME_FASTREFINE8)
		Flags &= ~H264_ME_HALFPELREFINE8_RD;

	if (Flags & H264_ME_FASTREFINE16)
		Flags &= ~H264_ME_HALFPELREFINE16_RD;
				 ...
				 const uint32_t iLimit)
{
	...
	int skip_thresh = INITIAL_SKIP_THRESH * \
		(current->vop_flags & H264_VOP_MODEDECISION_RD ? 2:1);
	...
	Data.qpel = (current->vol_flags & H264_VOL_QUARTERPEL ? 1:0);
	Data.chroma = MotionFlags & H264_ME_CHROMA_PVOP;
	Data.dctSpace = dct_space;
	Data.quant_type = !(pParam->vol_flags & H264_VOL_MPEGQUANT);
		...
		for (x = 0; x < mb_width; x++)	{
			...
			if (current->coding_type != S_VOP)	{ /* no fast SKIP for S(GMC)-VOPs */
				if (pMB->dquant == 0 && sad00 < pMB->quant * skip_thresh)
					if (Data.chroma || H264_me_SkipDecisionP(pCurrent, pRef, x, y, iEdgedWidth/2, pMB->quant)) {
			...
			if(MotionFlags & H264_ME_DETECT_STATIC_MOTION) {
				VECTOR *cmpMV;
				VECTOR staticMV = { 0, 0 };
			...
			if (current->vop_flags & H264_VOP_CARTOON) {
				if (current->coding_type == S_VOP) {
					...
					VECTOR predMV;
					if (current->vol_flags & H264_VOL_QUARTERPEL)
			...
			if (current->vop_flags & H264_VOP_MODEDECISION_RD)
				H264_me_ModeDecision_RD(&Data, pMB, pMBs, x, y, pParam,
								MotionFlags, current->vop_flags, current->vol_flags,
								pCurrent, pRef, pGMC, current->coding_type);

			else if (current->vop_flags & H264_VOP_FAST_MODEDECISION_RD)
				H264_me_ModeDecision_Fast(&Data, pMB, pMBs, x, y, pParam,
								MotionFlags, current->vop_flags, current->vol_flags,
								pCurrent, pRef, pGMC, current->coding_type);
			else
