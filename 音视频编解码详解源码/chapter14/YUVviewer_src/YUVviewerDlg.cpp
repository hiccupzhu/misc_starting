...
//一帧YUV图像的的大小是 nImageWidth*nImageHeight*3/2 (B)
UINT PlayVideo( LPVOID pParam )
{
	...
	}	//nPicShowOrder : 0--FrameEnd
	
	HANDLE hPlayTemp1 = OpenMutex(MUTEX_ALL_ACCESS,FALSE,"Play");
	
	while(g_nCurrentFrame >= g_nStartFrame &&
		  g_nCurrentFrame <= g_nEndFrame && !bEof)
	{
...
void CYUVviewerDlg::OnPauseplay() 
{
	...
	if ( m_pWinThread == NULL)
		m_pWinThread = AfxBeginThread( PlayVideo , this);//(AFX_THREADPROC)(void*)
...
{
	UpdateData(TRUE);//read data from control into the vars
...
/*
	// read rgb file
	if (picsize*3 != m_pFile[m_iCount]->ReadHuge(buf,picsize*3))
	{
		MessageBox("get to end of file");
	}
*/	
