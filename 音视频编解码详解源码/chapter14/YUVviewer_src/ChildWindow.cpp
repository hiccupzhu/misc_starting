...
{    
	...
	if (NULL== (Cb = (unsigned char *)malloc(iWidth*iHeight/2) ) ) 
	{
		AfxMessageBox("Couldn't allocate memory for Cb\n");
		return;
	}
	if (NULL== (Cr = (unsigned char *)malloc(iWidth*iHeight/2) ) ) 
	{
		AfxMessageBox("Couldn't allocate memory for Cr\n");
		return;
	}
...
void CChildWindow::ShowGrayImage(CDC *pDC,BYTE *lpImage)
{
    ...
    hMem=GlobalAlloc(GHND,iWidth*iHeight);	//GHND = GMEM_ZEROINIT | GMEM_MOVEABLE
...
void CChildWindow::ShowImage(CDC *pDC,BYTE *lpImage)
{
	...
	StretchDIBits(pDC->m_hDC,
				  ...
				  0,0,m_nzoom*iWidth,m_nzoom*iHeight,
				  // source rectangle
				  0,0,iWidth,iHeight,
				  lpImage, 
				  BmpInfo,
...
/*
	...
	DrawDibDraw(
			...
			rect.right,
			rect.bottom,
			(LPBITMAPINFOHEADER)BmpInfo,//(LPBITMAPINFOHEADER)bufo,
			lpImage,//&bufo[40],
			0,0,-1,-1,0
			);
*/
...
void CChildWindow::ConvertYuvToRGB24Bstream(LPBYTE pY,LPBYTE pCb,LPBYTE pCr,
											LPBYTE hpOutput,int imWidth,int imHeight)
{
	...
	for (y=0; y<imHeight; y++ ){
		cbuf = hpOutput + ((imWidth*3)*(imHeight-y-1));
		for (x=0; x<imWidth/2; x++){
			YUV2RGB(pY[0],pCb[0],pCr[0],r,g,b);
			cbuf[0] = (unsigned char)b;//TRIM(b);//
			cbuf[1] = (unsigned char)g;//TRIM(g);//
			cbuf[2] = (unsigned char)r;//TRIM(r);//
			YUV2RGB(pY[1],pCb[0],pCr[0],r,g,b);
			cbuf[3] = (unsigned char)b;//TRIM(b);//
			cbuf[4] = (unsigned char)g;//TRIM(g);//
			cbuf[5] = (unsigned char)r;//TRIM(r);//
			cbuf += 6;
			pY += 2;
			pCr++;
			pCb++;
...
//	nPicShowOrder++;
	...
	if(bColorImage){		//彩色	//Colorful
		...
		conv.YV12_to_RGB24(Y,Cb,Cr,RGBbuf,iWidth,iHeight);//4:2:0
		//ConvertYuvToRGB24Bstream(Y,Cb,Cr,RGBbuf,iWidth,iHeight);//4:2:2
		ShowImage(&dc,RGBbuf);
	}
	else {                  //黑白	//Monochrome
		ShowGrayImage(&dc,Y);
...
int CChildWindow::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	// the purpose is to init bmiColors field
